...
function user_access($module, $operation = DEFAULT_OPERATION, $account = NULL) {
    ...
    $capability = "";
    if (empty($operation)) {
        $qry = "select if(coalesce(Capability,'')='',0,OperationId) as Capability from sys_module_operation where ModuleName='$module'";
    } else {
        $qry = "select if(coalesce(Capability,'')='',0,OperationId) as Capability from sys_module_operation where ModuleName='$module' and Operation='$operation'";
    }
    ...
    /* echo $qry;
      ...
      print_r($rs);
      echo "<br>";
      print_r($account->perms); */
    if ($db->num_rows($rs) > 0) {
        while ($tmp = $db->fetch_array($rs)) {
...
class sqlDb {
    ...
    function sqlDb($dsn) {
        ...
        $this->linker = new mysqli($pdsn['host'], $pdsn['user'], $pdsn['pass'], $mysql_db) or die("Could not connect");
        $this->link = mysqli_connect($pdsn['host'], $pdsn['user'], $pdsn['pass'], $mysql_db, ini_get("mysqli.default_port")) or die("Could not connect " . mysqli_connect_error());
