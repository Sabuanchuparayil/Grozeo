    ...
    case 'listRetalinMarginItemMappping':
        ...
        if (isset($_POST['filter']) && $_POST['filter'] != '') {
            foreach ($_POST['filter'] as $key => $v) {
                $_allowedFilterFields = ['max_score'];
                ...
                switch ($v['data']['type']) {
                    case 'list':
                        if ($field == 'itemType') {
                            $search .= (($search == "") ? " where " : " and ") . 'rmim_isMedicine' . " = '" . ($safeValue == 'Medicine' ? '1' : '0') . "'";
                        } else {
                            $search .= (($search == "") ? " where " : " and ") . $field . " like '" . $safeValue . "%'";
                        }
                        break;
                        
                    case 'string':
                        if ($field == 'stit_SKU') {
                    $stit_SKU = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(rmim_stit_id),0) FROM retaline_margin_item_mapping WHERE rmim_stit_id IN (SELECT (stit_ID) FROM finascop_stock_itemmaster WHERE stit_SKU LIKE '{$safeValue}%' )");
                    $search .= " AND rmim_stit_id IN({$stit_SKU}) ";
                        } else if ($field == 'itemBC') {
                    $itemBC = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(rmim_stit_id),0) FROM retaline_margin_item_mapping WHERE rmim_stit_id IN (SELECT (stit_ID) FROM finascop_stock_itemmaster WHERE stit_brand_name LIKE '{$safeValue}%' OR dosform_name LIKE '{$safeValue}%')");
                    $search .= " AND rmim_stit_id IN({$itemBC}) ";
                        } else if ($field == 'itemMargin') {
                    $itemMargin = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(rmim_stit_id),0) FROM retaline_margin_item_mapping WHERE rmim_bmd_id IN (SELECT (bmd_id) FROM retaline_margindistributionsb2b WHERE bmd_name LIKE '{$safeValue}%' )");
