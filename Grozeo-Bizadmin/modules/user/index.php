...
switch ($op) {
    ...
    case 'getDetails':
        ...
        $resourceRoles = $db->getItemFromDB("SELECT GROUP_CONCAT(RoleId) FROM resource_role_mapping WHERE type = 0 AND UserId = {$id}");
        $resourceData = $db->getFromDB("SELECT * FROM resource_data WHERE UserId = {$id}", true);
        ...
        if ($tmp['user_type'] == 2) {
            $active_company = $db->getItemFromDB("SELECT GROUP_CONCAT(comp_id) from " . FINASCOP_DB . "finascop_user_activecompanies WHERE UserId = {$tmp['uidnr_admin']}");
        } else if ($tmp['user_type'] == 5) {
            $branch_office = $db->getItemFromDB("SELECT GROUP_CONCAT(boId) from " . FINASCOP_DB . "finascop_user_branchoffices WHERE UserId = {$tmp['uidnr_admin']}");
            if (!empty($branch_office))
                $office_company = $db->getItemFromDB("SELECT GROUP_CONCAT(boCompany) from " . FINASCOP_DB . "branch_office WHERE id IN ({$branch_office})");
        } else {
            $active_branch = $db->getItemFromDB("SELECT GROUP_CONCAT(br_Id) from " . FINASCOP_DB . "finascop_user_activebranches WHERE UserId = {$tmp['uidnr_admin']}");
            if (!empty($active_branch))
                $active_company = $db->getItemFromDB("SELECT GROUP_CONCAT(comp_id) from " . FINASCOP_DB . "finascop_branch_company WHERE br_Id IN ({$active_branch})");
        ...
        $data = array(
            ...
            "cmp_br[auditor_company]" => $auditor_company,
            "cmp_br[auditor_branch]" => $auditor_branch,
            "cmp_br[office_company]" => $office_company,
            "cmp_br[branch_office]" => $branch_office,
            "cmp_br[report_to_user]" => $report_to_user,
            "ad[is_auditor]" => $tmp['is_auditor'],
            "isreport_to_user" => $isreport_to_user,
            "primaryRole" => $tmp['id_admin_role'],
            "secondaryRole" => $resourceRoles,
            "usrRole[secondaryRole]" => $resourceRoles,
            "resource[dateofBirth]" => $resourceData['dateofBirth'],
            "resource[emergency_contact_name]" => $resourceData['emergencyContactName'],
            "resource[emergency_contact_number]" => $resourceData['emergencyContact'],
            "resource[emergency_relationship]" => $resourceData['relationship'],
            "resource[communication_address]" => $resourceData['address'],
            "resource[bloodgroup]" => $resourceData['bloodgroup'],
            "resource[roleDesignation]" => $resourceData['designationId'],
            "designationId" => $resourceData['designationId']
        );
    ...
    case 'getComboData':
        ...
        switch ($ind) {
            ...
            case 2:
                if ($_POST['comp_id'] > 0)
                    $qry = "SELECT br_ID as id,CONCAT(br_Name ,'-',branch_shortname) as `name` from " . FINASCOP_DB . "finascop_branch where br_ID in (select br_Id from " . FINASCOP_DB . "finascop_branch_company"
                        . " where comp_id IN ({$_POST['comp_id']}))";
                else
                    $qry = "SELECT br_ID as id,CONCAT(br_Name ,'-',branch_shortname) as `name` from " . FINASCOP_DB . "finascop_branch ";
                break;
            case 4:
            case 6:
                if ($_POST['comp_id'] > 0)
                    $qry = "SELECT b.br_ID AS id,CONCAT ((SELECT comp_shortname from " . FINASCOP_DB . "finascop_company WHERE comp_id = bc.comp_id),' [', b.br_Name ,'-',b.branch_shortname,']')  AS `name`
            ...
            case 8:
                if ($_POST['comp_id'] > 0)
                    $qry = "SELECT b.id AS id,CONCAT ((SELECT comp_shortname from  finascop_company WHERE comp_id = {$_POST['comp_id']}),' [', b.boName ,'-',b.boShortCode,']')  AS `name`
                from  branch_office b  where b.boCompany IN ({$_POST['comp_id']})";
                else
                    $qry = "SELECT b.id AS id,CONCAT ((SELECT comp_shortname from finascop_company WHERE comp_id = {$_POST['comp_id']}),' [', b.boName ,'-',b.boShortCode,']')  AS `name`
    ...
    case 'impersonate':
        ...
        if (empty($userID))
            return;
        $userMobile = $db->getItemFromDB("SELECT Telephone FROM finascop_usr_profile WHERE UserId = {$userID}");
        $isuserRegistered = $db->getItemFromDB("SELECT cust_id FROM retaline_customer WHERE cust_mobile = {$userMobile}");
        if ($isuserRegistered > 0) {

            $custRole = $db->getItemFromDB("SELECT defaultRole FROM retaline_customer WHERE cust_mobile = {$userMobile}");
            $data['cust_created_at'] = date('Y-m-d H:i:s');
            $data['cust_updated_at'] = date('Y-m-d H:i:s');
            if ($custRole == 'impersonate') {
                $data['defaultRole'] = 'user';
                $status3 = $db->perform('retaline_customer', $data, 'update', 'cust_id=' . $isuserRegistered);
                echo "{success:true,valid:true,message:'Impersonation removed ' }";
            } else {
                $data['defaultRole'] = 'impersonate';
                $status3 = $db->perform('retaline_customer', $data, 'update', 'cust_id=' . $isuserRegistered);
                echo "{success:true,valid:true,message:'Impersonation added ' }";
            }
        } else {
            $message = "This admin account does not have permissions to impersonate users, Please register as a customer using the mobile number " . $userMobile;
    ...
    case 'setAgentId':
        ...
        $userPrimaryLanguage = $_POST['userPrimaryLanguage'];
        $userSecondaryLanguage = $_POST['userSecondaryLanguage'];
        if (empty($_POST['enableSupport']))
            $enableSupport = 0;
        else
            $enableSupport = 1;
        if (empty($_POST['enableCallcenter']))
            $enableCallcenter = 0;
        else
            $enableCallcenter = 1;

        $data = array("agentID" => $userAgentId, "userDid" => $_POST['userDid'],"enableSupport" => $enableSupport,"enableCallcenter" => $enableCallcenter);
        $status = $db->perform("finascop_user_details", $data, "update", "UserId='$userID'");

        if ($userPrimaryLanguage > 0 && $userSecondaryLanguage > 0) {
            $isExistPrimary = $db->getItemFromDB("SELECT id FROM language_mapping WHERE  type = 0 AND typeId = {$userID} AND isfeatured = 1");
            if ($isExistPrimary > 0) {
                $pldata = array("languageId" => $userPrimaryLanguage);
                $status = $db->perform("language_mapping", $pldata, "update", "id='$isExistPrimary'");
            } else {
                $pldata = array("languageId" => $userPrimaryLanguage, "type" => 0, "isfeatured" => 1, "typeId" => $userID);
                $status = $db->perform("language_mapping", $pldata);
            }

            $isExistSecondary = $db->getItemFromDB("SELECT id FROM language_mapping WHERE  type = 0 AND typeId = {$userID} AND isfeatured = 0");
            if ($isExistSecondary > 0) {
                $sldata = array("languageId" => $userSecondaryLanguage);
                $status = $db->perform("language_mapping", $sldata, "update", "id='$isExistSecondary'");
            } else {
                $sldata = array("languageId" => $userSecondaryLanguage, "type" => 0, "isfeatured" => 0, "typeId" => $userID);
                $status = $db->perform("language_mapping", $sldata);
            }
            /* $existSecondary = [];
            $oldarray = [];
            $mergedArray = [];
            $existingSecondaryLanguage = [];
            $newSecondaryLanguage = explode(',', $userSecondaryLanguage);
            $existSecondary = $db->getMultipleData("SELECT id,languageId FROM language_mapping WHERE  type = 0 AND typeId = {$userID} AND isfeatured = 0", true);
            $existingSecondaryLanguage = array_column($existSecondary, 'languageId');
            $languagesToRemove = array_diff($existingSecondaryLanguage, $newSecondaryLanguage);
            // Remove elements from old array that are not in new array
            $oldarray = array_intersect($existingSecondaryLanguage, $newSecondaryLanguage);
            // Merge the remaining old array with the new array
            if (count($oldarray) > 0) {
                $mergedArray = array_merge($newSecondaryLanguage, $oldarray);
            } else {
                $mergedArray = $newSecondaryLanguage;
            }
            $finalLanguages = array_unique($mergedArray);
            foreach ($languagesToRemove as $remLan) {
                $status = $db->query("DELETE FROM language_mapping WHERE  type = 0 AND typeId = {$userID} AND isfeatured = 0 AND languageId = {$remLan}");
            }
            foreach ($finalLanguages as $finalLanguage) {
                $isExistSecLang = $db->getItemFromDB("SELECT count(id) FROM language_mapping WHERE  type = 0 AND typeId = {$userID} AND isfeatured = 0 AND languageId = {$finalLanguage}");
                if ($isExistSecLang == 0) {
                    $sldata = array("languageId" => $finalLanguage, "type" => 0, "isfeatured" => 0, "typeId" => $userID);
                    $status = $db->perform("language_mapping", $sldata);
                }
            }*/
        }
        if ($status) {
            echo "{success:true,valid:true,message:'Languages updated ' }";
        } else {
        ...
        break;
    case 'storeForlanguages':
        $cond = " ";
        $PrimaryLanguage = $_POST['PrimaryLanguage'];
        if ($PrimaryLanguage > 0) {
            $cond = " WHERE id <> {$PrimaryLanguage} AND isPreferred = 1 ";
        }
    ...
    case 'role_designation':
        ...
        $roleId = $_POST['roleId'];
        if ($roleId > 0) {
            $cond = " WHERE roleId = {$roleId}";
        }
        $qry = "SELECT org_designation.id AS desigId,org_designation.`name` AS desigName FROM org_designation 
        /*INNER JOIN role_designation_mapping ON designationId = org_designation.id*/ {$cond} order by name";
        $data = $db->getMultipleData($qry, true);
        if (!empty($data)) {
            echo json_encode($data);
        } else
    ...
    case 'role_secondary':
        ...
        if ($primaryRoleId > 0) {
            $cond = " AND RoleID NOT IN (1,{$primaryRoleId})";
        }
        $qry = 'SELECT RoleId AS id_admin_role,RoleName AS admin_role_name FROM sys_role WHERE IsEnabled = "Yes" ' . $cond . ' ORDER BY RoleName ASC ';
        $data = $db->getMultipleData($qry, true);
        if (!empty($data)) {
            echo json_encode($data);
        } else
