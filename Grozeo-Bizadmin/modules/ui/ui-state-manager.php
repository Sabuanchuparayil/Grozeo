...
$clientArgs->id = (int)(isset($_POST["id"]) ? $_POST["id"] : 1);
$clientArgs->user = (int)(isset($_POST["user"]) ? $_POST["user"] : $_SESSION['admin']->Finascop_UserId);
$clientArgs->session = preg_replace('/[^a-zA-Z0-9,-]/', '', isset($_POST["session"]) ? $_POST["session"] : session_id());
...
if($cmd == "saveSessionState"){
    $clientArgs->viewname    = htmlspecialchars(trim($_POST["ViewName"] ?? ''), ENT_QUOTES, 'UTF-8');
    $clientArgs->description = htmlspecialchars(trim($_POST["view_description"] ?? ''), ENT_QUOTES, 'UTF-8');
    $clientArgs->ispublic    = ($_POST["is_public"]=="on") ? 'Yes':'No';
	$clientArgs->siteid		 = (int)($_POST["current_site_id"] ?? 0);
}
...
$allowedCmds = ['readState', 'saveState', 'saveSessionState', 'getViewsStore', 'deleteTempState', 'getViewsDependingOnWorkflow'];
if (!in_array($cmd, $allowedCmds, true)) {
...
function getViewsStore($db, $clientArgs) {
   ...
   $defaultViewId = $db->getItemFromDB('SELECT Varvalue FROM usr_preference WHERE Varname = "default_view" AND UserId =' . $sessionUserId);
    ...
    if(!empty($_POST['query'])){
        $whr = "  AND (ViewName LIKE '" . addslashes($_POST['query']) . "%')";
    }
