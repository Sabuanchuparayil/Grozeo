...
switch ($op) {
    ...
    case 'getPreferredAreas':
        if (!empty($_POST['locationGlatitude']) && !empty($_POST['locationGlongitude'])) {
            ...
            FROM `area_entries` WHERE 1=1 HAVING distance <=50 
    ORDER BY distance ASC";
        } else {
            $qry = "SELECT id,areaName FROM `area_entries` WHERE 1=1 ";
        }
    ...
    case 'loadEditData':
        ...
        updatedOn,cp.name as professionName,country_code,(SELECT areaId FROM consulting_partner_areas WHERE consultingPartnerId = caa.id AND type = 3) as preferredArea  FROM crm_consulting_partner caa LEFT JOIN crm_proffession cp ON cp.id = professionId WHERE caa.id={$aaId}";
            $results = $supportdb->getFromDB($QRY, true);
            if ($results['preferredArea'] == -1) {
                $location = $supportdb->getFromDB("SELECT stateId,districtId FROM consulting_partner_districts WHERE consultingPartnerId = {$results['aaId']} AND type = 3", true);
                $results['as_st_id'] = $location['stateId'];
                $results['as_dst_Id'] = $location['districtId'];
            }
    ...
    case 'insertContactAndMoveToLead':
        ...
        if (
            ...
            !empty($data['gstNumber']) && !empty($data['businessLocation']) && !empty($data['locationGlatitude']) && !empty($data['locationGlongitude']) && (!empty($preferredArea) || !empty($as_dst_Id))
        ) {
            $msg = "Influencer Converted.";
            $data['entryType'] = 2;
        } else {
            $msg = "Influencer Created.";
            $data['entryType'] = 1;
        }

        $supportdb->query('begin');
        if ($id > 0) {
            $isUnique = $supportdb->getItemFromDB("SELECT COUNT(*) FROM crm_consulting_partner WHERE mobile ='{$_POST['aaMobile']}' AND id <> {$id}");
            if ($isUnique > 0) {
                echo "{success: false,errors: { msg: 'Edited details are already existing.' }}";
                exit;
            } else {
    ...
    case 'getContactDetails':
        ...
        $allowedFields = ['inf_name', 'inf_platform', 'inf_status'];
        ...
        $qry = "SELECT caa.id as aaId,caa.name as aaName,mobile as aaMobile,email as aaEmail,professionId,businessVertical,organisationName,organisationType,
        gstNumber,businessLocation,locationGlatitude,locationGlongitude,createdBy,createdOn,updatedBy,
        updatedOn,cp.name as professionName,baId  FROM crm_consulting_partner caa left join crm_proffession cp on cp.id = professionId {$search} ORDER BY aaId desc  LIMIT $rec_start,$rec_limit";
    ...
    case 'convertToContact':
        ...
        $mobileAvailable  = $supportdb->getItemFromDB("SELECT mobile FROM crm_consulting_partner WHERE id = {$id} ");
        if (empty($mobileAvailable)) {
            echo "{success: false,msg:'Contact details are missing in this enqiry'}";
            exit();
        }
        $isUnique = $supportdb->getItemFromDB("SELECT COUNT(*) FROM crm_consulting_partner WHERE mobile ='{$mobileAvailable}' AND entryType = 1 AND id = {$id} ");
        if ($isUnique > 0) {
...
END AS typeName from crm_cpcommunications cc LEFT JOIN crm_stages stg on stg.id = stageId WHERE consultingPartnerId = {$consultingPartnerId}";
    ...
    case 'approveInfluencer':
        ...
        //if ($areaId > 0) {
        $userExists = $db->getItemFromDB("SELECT COUNT(*) FROM business_associate WHERE baMobileNo = {$aaDetails['mobile']}");
        if ($userExists > 0) {
            echo "{success: false,msg: 'User already exists in this number.' }";
            exit;
        }
        $supportdb->query('begin');
        ...
        $baData['refid'] = $refid;
        $baData['userType'] = 8;
        $baData['st_id'] = $locationDetails['stateId'];
        $baData['dst_Id'] = $locationDetails['districtId'];
        $baData = array_filter($baData);
        $status = $db->perform("business_associate", $baData);
        $baId = $db->insert_id();
        ...
        $badata['baId'] = $baId;
        $badata['updatedOn'] = date('Y-m-d H:i:s');
        $badata['updatedBy'] = $_SESSION['admin']->Finascop_UserId;
        $status = $supportdb->perform("crm_consulting_partner", $badata, 'update', " id = {$consultingPartnerId}");
