...
//        }
        $db->query('begin');
        if ($adhocCount > 0) {
            $updatedDate = $db->getItemSafe("SELECT prereq_updatedon FROM finascop_purchase_order_poprereq WHERE prereq_uniqueid = '?'", "i", [$_POST['fpopredet_uniqueid']]);
        ...
        $newupdatedDate = $db->getItemSafe("SELECT prereq_updatedon FROM finascop_purchase_order_poprereq WHERE prereq_uniqueid = '?'", "i", [$_POST['fpopredet_uniqueid']]);
...
if ($_SESSION['admin']->br_PyramidLevel == 1) {
 ...
 FROM finascop_purchase_order_poprereq fp  {$filter_qry} where prereq_uniqueid = '?'", "i", [$_POST['uniqueid']], true);
        if (!empty($podata)) {
