...
switch ($op) {
    case 'getReturnedOrders':
        ...
        if ($current_branch_id > 0) {
            ...
            if ($pyramidLevel == 4) {
                ...
                $listQuery = " SELECT DISTINCT(finascop_stock_return_request_order.order_id),rtrqo_id,bco.order_id,order_order_id,order_packedbags_count,order_customer_id,order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,
            ...
            FROM finascop_stock_return_request_order INNER JOIN retaline_customer_order bco ON bco.order_id = finascop_stock_return_request_order.order_id
                ...
                $listQuery = " SELECT DISTINCT(rsc.order_id),rtrqo_id,bco.bbso_id as order_id,bbso_SONumber as order_order_id,bbso_packedbags_count as order_packedbags_count,
                    b2b_Customer_ID as order_customer_id,bco.br_ID as order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,
        ...
        if ($ordertype == 'B2B') {
            ...
            if ($_POST['customer_mobileretreq'] != "") {
                ...
                $countQuery = "SELECT COUNT(*) " . " FROM retaline_B2B_SalesOrder bco
                        ...
                        INNER JOIN retaline_B2B_Status bcos ON bcos.status_id = bco.status_id  
                        INNER JOIN finascop_branch ON finascop_branch.br_ID = bco.br_ID INNER JOIN retaline_B2Bcustomer rc ON rc.b2b_Customer_ID = bco.b2b_Customer_ID" . " WHERE 1=1  AND bco.status_id IN(9,29) AND bbso_HasReturnRequest = 0 AND bco.br_ID = {$_SESSION['admin']->finascop_current_branch_id} {$mob_order} {$filter_qry}  {$order}";

                $listQuery = " SELECT bbso_id as order_id,bbso_SONumber as order_order_id,bco.b2b_Customer_ID as order_customer_id,bco.br_ID as order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,DATE_FORMAT(bbso_createdon,'%d-%m-%Y') AS order_created_on,
            ...
            if ($_POST['customer_mobileretreq'] != "") {
                ...
                $countQuery = "SELECT COUNT(*) " . " FROM retaline_customer_order bco
                        INNER JOIN retaline_customer_order_status bcos ON bcos.status_id = bco.status_id  
                        INNER JOIN finascop_branch ON br_ID = order_branch_id INNER JOIN retaline_customer rc ON cust_id = order_customer_id" . " WHERE 1=1  AND bco.status_id IN(17,18) AND order_HasReturnRequest = 0 AND order_branch_id = {$_SESSION['admin']->finascop_current_branch_id} {$mob_order} {$filter_qry}  {$order}";
    ...
    case 'listReturnDamage':
        ...
        if ($_SESSION['admin']->br_PyramidLevel == 1) {
            ...
            /*if ($_SESSION['admin']->IsSuperUser == 'Yes') {
                ...
                $brQuery = " AND rtrqo_sourceBranch = {$current_branch_id} ";
            }*/
        } else {
            $brQuery = " AND rtrqo_sourceBranch = {$current_branch_id} ";
        }
        ...
        $damagedItems = "SELECT GROUP_CONCAT(finascop_stock_return_request_order_details.rtrqo_id) AS rtrqoId,rtrqod_item_id,
                    SUM(rtrqod_return_damaged) AS item_count,finascop_stock_return_request_order_details.rtrqo_type 
                    from finascop_stock_return_request_order_details 
                    INNER JOIN finascop_stock_return_request_order ON finascop_stock_return_request_order.rtrqo_id = finascop_stock_return_request_order_details.rtrqo_id  
                    WHERE 1=1 {$brQuery} AND rtrqod_isPackOrderCreated = 0 AND rtrqod_return_damaged > 0  GROUP BY rtrqod_item_id,finascop_stock_return_request_order_details.rtrqo_type ORDER BY rtrqod_item_id DESC ";
    ...
    case 'createPackingOrderfromReturns':
        ...
        $uid_max = getNewTONumber($_SESSION['admin']->finascop_current_branch_id);
    ...
    case "saveItemReturninOrder":
        ...
        if ($ordertype == 'B2B') {
            ...
            $item_retail_price = $db->getItemFromDB("SELECT item_retail_price FROM retaline_customer_order_items WHERE customer_order_id = {$order_no} AND item_product_id = {$item_product_id}");
            $orderItemCount = $db->getItemFromDB("SELECT SUM(item_order_qty) FROM retaline_customer_order_items WHERE customer_order_id = {$order_no}");
            $order_roundoff = $db->getItemFromDB("SELECT order_roundoff FROM retaline_customer_order WHERE order_id = {$order_no}");
            $order_delivery_charge = $db->getItemFromDB("SELECT order_delivery_charge FROM retaline_customer_order WHERE order_id = {$order_no}");
            ...
            if ($returned_qty > 0) {
                ...
                $stit_id = $db->getFromDB("SELECT mrp,selling_price,fsbg_id FROM finascop_stock_branch_inventory WHERE stit_id = {$item_product_id} AND branch_id = {$order_branch_id} AND mrp = {$item_retail_price}", true);
                //$fsbg_id = $db->getItemFromDB("SELECT fsbg_id FROM finascop_stock_item_inventorydetails WHERE stiid_barcode = {$barcodesearch_fieldrt}");

                if ($itemReturn_type1qty > 0) {
                    if ($itemCountEntry > 0) {
                        //fsbg_id = {$fsbg_id} AND
                        $status = $db->query("UPDATE finascop_stock_branch_inventory SET item_count = item_count + {$itemReturn_type1qty} WHERE  stit_id = {$item_product_id} AND branch_id = {$order_branch_id} AND mrp = {$stit_id['mrp']} ");
                    } else {
...
*/
    ...
    case 'listReturnedToDamage':
        ...
        $count = $db->getItemFromDB($countQuery);
        $listQuery = "SELECT fsb.stit_id as stit_id,br.CONCAT(br_Name ,'-',branch_shortname) as br_Name,fsi.stit_SKU as stit_SKU,fsbg_id,frbi_epr,frbi_leastSKUmrp,frbi_leastSKUepr," . "fsb.item_count as item_count,fsb.mrp as mrp,fsb.selling_price as selling_price,fsb.branch_id as branchId,stit_category_name,stit_quantity,stit_brand_name,stit_product_variant,frbi_status,frbi_id," . "CASE WHEN frbi_status= 1 THEN 'Damaged' WHEN frbi_status= 2 THEN 'Expirable' END AS frbi_statusName," . "CASE WHEN br.br_stockLevel= 1 THEN fsi.stitl1_optimumqty " . "WHEN br.br_stockLevel= 2 THEN fsi.stitl2_optimumqty " . "ELSE stitl3_optimumqty END AS optimumqty," . "CASE WHEN br.br_stockLevel= 1 THEN fsi.stit11_minimumqty " . "WHEN br.br_stockLevel= 2 THEN fsi.stit12_minimumqty " . "ELSE stit13_minimumqty END AS minimumqty," . "CASE WHEN br.br_stockLevel= 1 THEN fsi.stit11_maximumqty " . "WHEN br.br_stockLevel= 2 THEN fsi.stit13_maximumqty " . "ELSE stit13_maximumqty END AS maximumqty " . "FROM finascop_return_branch_inventory fsb " . "INNER JOIN finascop_stock_itemmaster fsi ON fsb.stit_id=fsi.stit_ID " . "INNER JOIN finascop_branch br ON br.br_ID=fsb.branch_id {$search}{$where}{$searchitem} AND fsb.item_count > 0 ORDER BY $rec_sort $rec_sort_dir LIMIT $rec_start,$rec_limit";
        $datas = $db->getMulipleData($listQuery, true);
        $resCount = count($datas);
        for ($i = 0; $i < $resCount; $i++) {
            //echo '$i'.$stii_epraft;
    ...
    case 'listbarcodesinCurrentStock':
        ...
        if (isset($_POST['filter'])) {
            ...
            foreach ($_POST['filter'] as $key => $val) {
                $_allowedFilterFields = ['stiid_barcode'];
                if (!in_array($val['field'], $_allowedFilterFields)) continue;
                $safeValue = $db->sanitizeString($val['data']['value']);
                if ($val['field'] == 'stiid_barcode') {
                    $filter_part .= " and " . $val['field'] . " = " . $safeValue . " ";
                } else {
                    $filter_part .= " and " . $val['field'] . " LIKE '%" . $safeValue . "%' ";
                }
            }
        }
    ...
    case 'getReturnRequestOrders':
        ...
        if ($current_branch_id > 0) {
            ...
            $countQuery = "SELECT COUNT(*) " . " FROM retaline_customer_order bco  LEFT JOIN finascop_stock_return_request_order   ON bco.order_id = finascop_stock_return_request_order.order_id 
                        INNER JOIN retaline_customer_order_status bcos ON bcos.status_id = bco.status_id  
                        INNER JOIN finascop_branch ON br_ID = order_branch_id " . " WHERE finascop_stock_return_request_order.order_id is null  and bco.status_id > 0 AND order_HasReturnRequest = 1 AND order_branch_id = {$current_branch_id}   {$filter_qry}  ORDER BY bco.order_id desc";

            $listQuery = " SELECT DISTINCT(finascop_stock_return_request_order.order_id),rtrqo_id,bco.order_id,order_order_id,order_packedbags_count,order_customer_id,order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,
                DATE_FORMAT(created_at,'%d-%m-%Y') AS order_created_on,TIME_FORMAT(cast(created_at as time),'%r') as ordertime,admin_description AS order_status,
            (SELECT cust_customer_name FROM `retaline_customer` WHERE cust_id = order_customer_id) AS delivery_to,rtrqo_sourceType,
            (SELECT cust_mobile FROM `retaline_customer` WHERE cust_id = order_customer_id) AS cust_mobile,order_itemReturnRequestCount,
            order_HasReturn,order_ItemsReturned,order_ReturnVerified,DATE_FORMAT(updated_at,'%Y-%m-%d %H:%i:%s') as updated_at 
            FROM retaline_customer_order bco LEFT JOIN finascop_stock_return_request_order  ON bco.order_id = finascop_stock_return_request_order.order_id
                        INNER JOIN retaline_customer_order_status bcos ON bcos.status_id = bco.status_id  
                        INNER JOIN finascop_branch ON br_ID = order_branch_id  " . " WHERE finascop_stock_return_request_order.order_id is null and bco.status_id > 0  AND order_HasReturnRequest = 1 AND order_branch_id = {$current_branch_id}  {$filter_qry}  GROUP BY bco.order_id ORDER BY bco.order_id desc limit " . $start . "," . $limit;
        ...
        echo '{"totalCount":"', $count, '","data":' . json_encode($datas) . '}';
        break;
    case "return_request_details":
        require(THIS_MODULE_PATH . "/return_request_details.php");
        break;
}
