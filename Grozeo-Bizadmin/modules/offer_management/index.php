...
switch ($op) {
    case 'listOfferMgmt':
        ...
        $qry = "SELECT bom_id,bom_bmdCompany,bom_bmdHub,bom_bmdIncentive,bom_bmdTechnology,bom_bmdCustomer,bom_offrPlacement,bom_offrPromotion,bom_offrSupplier,stiid_fpoid,stiid_itemmasterid,bom_status,bom_locked,"
            ...
            . " FROM retaline_offer_management WHERE 1=1 AND bom_type = 0 {$search}";
    ...
    case 'listCouponOfferMgmt':
        ...
        $qry = "SELECT bom_id,
CASE
WHEN discountType = 1 THEN 'Flat Discount'
WHEN discountType = 2 THEN 'Invoice Target'
WHEN discountType = 3 THEN 'Product Discount'
WHEN discountType = 4 THEN 'Buy X-Get Y'
WHEN discountType = 5 THEN 'Delivery Discount'
END AS bom_typeName,
CASE
...
FROM retaline_offer_management WHERE 1=1 {$filter_qry} ORDER BY $sort $dir limit {$start},{$limit}";
        $countQuery = "SELECT COUNT(*) FROM retaline_offer_management WHERE 1=1 {$filter_qry}";
        $count = $db->getItemFromDB($countQuery);
        $data = $db->getMultipleData($qry, true);
        if (!empty($data)) {
            echo '{"totalCount":' . $count . ',"data":' . json_encode($data) . '}';
    ...
    case 'ofperDetailsViewCoupon':
        $bom_id = isset($_POST['bom_id']) ? intval($_POST['bom_id']) : 0;
        if ($bom_id > 0) {
            $result = $db->getFromDB("SELECT discountType,bom_bmdCompany,bom_bmdHub,bom_bmdIncentive,bom_bmdTechnology,bom_bmdCustomer,bom_offrPlacement,bom_offrPromotion,bom_offrSupplier,bom_narration,bom_subCategoryId,stiid_itemmasterid,bom_type,bom_offrDiffer,"
            ...
            switch ($result['discountType']) {
                case '1':
                    $typeName = 'Flat Discount';
                    break;
                case '2':
                    $typeName = 'Invoice Target';
                    break;
                case '3':
                    $typeName = 'Product Discount';
                    break;
                case '4':
                    $typeName = 'Buy X-Get Y';
                    break;
                case '3':
                    $typeName = 'Delivery Discount';
                    break;
            }
            //0:Offer Percentage,1:Coupon Offers,2:Amount
            switch ($result['bom_type']) {
                case '0':
                    $offerType = 'Offer Percentage';
                    break;
                case '1':
                    $offerType = 'Coupon Offers';
                    break;
                case '2':
                    $offerType = 'Amount';
                    break;
            ...
            $result['targetAmount'] = $result['bom_offrDiffer'];
