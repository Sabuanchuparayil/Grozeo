...
//                        $filter_part .= " AND id  IN({$cpd_name}) ";
        ...
        $query = "SELECT id,name, 
            ...
            CASE  WHEN is_cpd = 1 THEN 'CPD' 
                    ...
                    IF((branch_type_id=1),(SELECT CONCAT(br_Name,' - ',branch_shortname) FROM finascop_branch cp WHERE br_ID = cpdid),(SELECT stpa_Fname FROM finascop_stock_party  WHERE stpa_id = cpdid))  as cpd_name
                    from retaline_godown_boy gb ";
        //(SELECT br_Name FROM finascop_branch cp WHERE br_ID = cpdid)  AS cpd_name
        $countQuery = "SELECT COUNT(*) from ({$query}) as gcount WHERE {$filter_part} ORDER BY $rec_sort $rec_sort_dir ";
    ...
    case 'savegodownExecutive':
        ...
        if (!empty($_POST['gdwn_party'])) {
            $branchname = $_POST['gdwn_party'];
            $gdwn_party = 2;
        } else {
            $gdwn_party = 1;
        ...
        $data = array(
            ...
            "allowStoreClose" => $_POST['allowStoreClose'],
            "allowInventoryControl" => $_POST['allowInventoryControl'],
            "branch_type_id" => $gdwn_party
        );
        $db->query('begin');
        if (empty($_POST['id'])) {
    ...
    case 'loadgodownExecutive':
        ...
        if ($id) {
            ...
            $qry = $db->getFromDB("SELECT name,lname,emp_id,emp_ni_number,emp_email_id, emp_add1, emp_add2,emp_pincode,
                phone,branch_id as branch_name,is_allowManualSchedule,is_allowAutoSchedule,branch_type_id,allowStoreClose,allowInventoryControl,
                IF(branch_type_id = 2,(SELECT stpa_Fname FROM finascop_stock_party  WHERE stpa_id = branch_id),' ') AS gdwn_partyname,
                IF(branch_type_id = 1,(SELECT br_Name FROM finascop_branch  WHERE br_ID = branch_id),' ') AS cpd_branch 
 FROM retaline_godown_boy WHERE id = $id", true);
    ...
    case 'getBranchName':
        ...
        $qry = $db->getMulipleData("SELECT br_ID,CONCAT(br_Name,' - ',branch_shortname) AS br_Name FROM finascop_branch where br_status = 'Active' and br_PyramidLevel IN (2,3,4)", true);
...
FROM retaline_godown_boy_orders WHERE bgor_id = {$datas[$i]['id']} ";
    ...
    case 'chStatusGodownExecutive':
        ...
        $status = $db->perform('retaline_godown_boy', $data, 'update', "id = " . intval($_POST['id']));
        $status = $db->query('commit');
        if ($status == 1) {
