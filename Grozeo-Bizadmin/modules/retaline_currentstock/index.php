    ...
    case 'getLiveStock':
        ...
        if (isset($_POST['filter']) && $_POST['filter'] != '') {
            ...
            foreach ($_POST['filter'] as $key => $v) {
                $_allowedFilterFields = ['stit_SKU', 'mrp', 'stit_id', 'branch_id'];
                ...
                switch ($v['data']['type']) {
                    case 'string':
                        $filter[$field]['data']['value'] = $safeValue;
                    case 'numeric':

                        switch ($v['data']['comparison']) {
                            case 'gt' :
                                $filter[$field]['data']['comparison']['gt'] = 'gt';
                                $filter[$field]['data']['value']['gt'] = $safeValue;
                                break;
                            case 'lt':
                                $filter[$field]['data']['comparison']['lt'] = 'lt';
                                $filter[$field]['data']['value']['lt'] = $safeValue;
                                break;
                            case 'eq':
                                $filter[$field]['data']['comparison']['eq'] = 'eq';
                                $filter[$field]['data']['value']['eq'] = $safeValue;
                                break;
                        }
        ...
        if (isset($filter)) {
            ...
            foreach ($filter as $key => $field) {
                ...
                if ($field['field'] == 'Availability') {
                    if (strpos($field['data']['value'], 'y') === 0) {
                        $searchitem .= " and (finascop_branch.isAvailable  = 1) ";
                    }
                } else if ($field['field'] == 'OnDemand') {
                    if (strpos($field['data']['value'], 'y') === 0) {
                        $searchitem .= " and (finascop_branch.isOnDemand  = 1) ";
                    }
                } else {
...
LEFT JOIN mypha_productmanufacture mpm ON mpm.manufacture_id=mpb.manufacture_id "
                . "INNER JOIN finascop_branch br ON br.br_ID=fsb.branch_id {$search}{$where}{$searchitem}";
        $count = $db->getItemFromDB($countQuery);

        $listQuery = "SELECT id,fsb.stit_id as stit_id,CONCAT(br.br_Name,' - ',br.branch_shortname) AS br_Name,CONCAT(fsi.stit_SKU,'-',fsb.mrp) as stit_SKU,fsbg_id,purchasing_unit,stit_ParentItemId,isAvailable,"
                . "fsb.item_count as item_count,fsb.mrp as mrp,fsb.selling_price as selling_price,fsb.branch_id as branchId,isOnDemand,"
                ...
                . "product_category,stit_category_name,stit_brand_name,med_manufacturename,category_name,mppc.parent_category as parent_category  "
                . "FROM finascop_stock_branch_inventory fsb "
                . "INNER JOIN finascop_stock_itemmaster fsi ON fsb.stit_id=fsi.stit_ID "
                . "INNER JOIN mypha_productsubcategory mpsc ON mpsc.sub_category_id=fsi.product_category
INNER JOIN mypha_productcategory mpc ON mpc.category_id=mpsc.main_category  
...
LEFT JOIN mypha_productmanufacture mpm ON mpm.manufacture_id=mpb.manufacture_id "
        ...
        if (!empty($datas)) {
            for ($i = 0; $i < $resCount; $i++) {
                ...
                if ($datas[$i]['isOnDemand'] == 0) {
                    $datas[$i]['OnDemand'] = 'No';
                } else {
                    $datas[$i]['OnDemand'] = 'Yes';
                }
                if ($datas[$i]['isAvailable'] == 0) {
                    $datas[$i]['Availability'] = 'No';
                } else {
                    $datas[$i]['Availability'] = 'Yes';
                }
            }
...
// $db->printGridJson($countQuery, $listQuery);
    ...
    case 'getBranchName':
        ...
        //AND (br_cpd={$branch_id} OR br_ID={$branch_id})
        if (!empty($qry)) {
            echo json_encode($qry);
        } else
            echo [];
...
//pass branchid and item list details from stock inventory details where sttus id in 1,4
    ...
    case 'listbarcodesMovement':
        ...
        break;
    case 'addItemasOnDemand':
        $id = $_POST['id'];
        $isOnDemand = $_POST['isOnDemand'];
