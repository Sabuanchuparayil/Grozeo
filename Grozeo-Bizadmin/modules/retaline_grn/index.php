...
//        }
        $db->query('begin');
        if ($adhocCount > 0) {
            $retgrn_id = $db->getItemSafe("SELECT retgrn_id FROM retaline_grn WHERE retgrn_uniqueid = '?'", "i", [$_POST['retgrnd_uniqueid']]);
            $updatedDate = $db->getItemSafe("SELECT retgrn_updatedon FROM retaline_grn WHERE retgrn_uniqueid = '?'", "i", [$_POST['retgrnd_uniqueid']]);
        ...
        $data['retgrnd_itemmrp'] = $_POST['retgrnd_itemoffrrate'];
        ...
        $newupdatedDate = $db->getItemSafe("SELECT retgrn_updatedon FROM retaline_grn WHERE retgrn_uniqueid = '?'", "i", [$_POST['retgrnd_uniqueid']]);
...
if ($_SESSION['admin']->br_PyramidLevel == 1) {
 ...
 FROM retaline_grn fp  {$filter_qry} where retgrn_uniqueid = '?'", "i", [$_POST['uniqueid']], true);
        if (!empty($podata)) {
