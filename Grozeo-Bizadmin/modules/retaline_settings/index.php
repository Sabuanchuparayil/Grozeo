...
switch ($op) {
    ...
    case 'listPage':
        ...
        $listQuery = "SELECT page_id,page_name,page_content,IF((page_status=1),'Active','Inactive') AS page_status,
        CASE WHEN page_type=1 THEN 'About' WHEN page_type=2 THEN 'Privacy Policy' WHEN page_type=3 THEN 'Terms' WHEN page_type=4 THEN 'How it Works' END AS page_type,
        IF(storegroup_id >0,(SELECT store_group_name FROM finascop_branch_group WHERE store_group_id = storegroup_id),'Default') AS sgName FROM app_pages 
        " . "{$search} {$searchitem} ORDER BY {$sort} {$dir} limit $rec_start,$rec_limit";
    ...
    case 'page_form_load':
        ...
        $page_id = isset($_POST['page_id']) ? intval($_POST['page_id']) : 0;
        if ($page_id) {
            $sql = "SELECT  page_id,page_name,page_content,page_status,page_type AS page_type FROM app_pages WHERE page_id =" . $page_id;
            $results = $db->getFromDB($sql, true);
            if (!$results) {
                echo '{"success":true,"data":[]}';
            } else {
    ...
    case 'savePage':
        ...
        if ($data['page_id'] > 0) {
            ...
            $storegroup_id = $db->getItemFromDB("SELECT storegroup_id FROM app_pages WHERE page_id = '{$page_id}'");
            $PageUnique = $db->getItemFromDB("SELECT COUNT(*) from app_pages WHERE page_type ='{$page_type}' AND storegroup_id = {$storegroup_id} AND page_id!='{$page_id}' ");
            if ($PageUnique > 0) {
            ...
            $PageUnique = $db->getItemFromDB("SELECT COUNT(*) from app_pages WHERE page_type ='{$page_type}' AND storegroup_id = 0 ");
            if ($PageUnique > 0) {
                echo "{success: false, message:'This name already existing.'}";
                exit;
            } else {
    ...
    case 'listfeedback':
        ...
        $search = " WHERE 1=1 AND status IN (1,2) ";
        if (isset($data['filter'])) {
            $filter_qry .= buildSafeFilterQuery($data['filter'], $allowedFields, $db);
        }
        $countQuery = "SELECT COUNT(*) FROM app_feedback  {$search}";
        $listQuery = "SELECT fb_id,fb_mobile,fb_email,fb_comments,DATE_FORMAT(fb_createdOn, '%Y-%m-%d %H:%i:%s') AS fb_createdOn,
        storegroup_id,status,CASE WHEN status=1 THEN 'Active'  WHEN status=2 THEN 'Attended' WHEN status=3 THEN 'Removed'  END AS statusName,
        (SELECT store_group_name FROM finascop_branch_group WHERE finascop_branch_group.store_group_id = app_feedback.storegroup_id) AS storegroupName 
        FROM app_feedback {$search}  ORDER BY {$sort} {$dir} limit $start,$limit";
    ...
    case 'listnotification':
        ...
        $listQuery = "SELECT notification_id,notification_content,IF((notification_status=1),'Active','Inactive') AS notification_status,notification_remarks FROM app_notification {$search}  ORDER BY {$sort} {$dir} limit $start,$limit";
    ...
    case 'notificationDetailsView':
        ...
        if ($notification_id || $ID) {
            ...
            $data = $db->getFromDB("SELECT notification_id,notification_content,notification_status,notification_remarks FROM app_notification WHERE notification_id  =" . $notification_id, true);
            $data['success'] = true;
            echo json_encode($data);
        }
        break;
    ...
    case 'saveCourier':
        ...
        if ($data['mst_courier_id'] > 0) {
            ...
            if ($CourierUnique > 0) {
                ...
                $data['accled_ReferenceId'] = getNewFinascopApiKey();
    ...
    case 'listStoreGroups':
        $grosmartMerchant = $_POST['grosmartMerchant'];
        ...
        if($grosmartMerchant > 0){
            $searchitem .= " AND store_group_grosmartMerchant = 1 ";
        }
        if (isset($data['filter'])) {
            $filter_qry .= buildSafeFilterQuery($data['filter'], $allowedFields, $db);
        }
        $countQuery = "SELECT COUNT(*) FROM finascop_branch_group  {$search}";
        $listQuery = "SELECT a.store_group_id,store_group_name,IF((status=1),'Active','Inactive') AS status,siteUrl,SUBSTRING_INDEX(siteUrl, '.', 1) AS permalink,SUBSTRING_INDEX(siteUrl, '.', -2) AS domain, '' as customDomain,
            (SELECT COUNT(*) FROM finascop_branch WHERE br_storeGroup = store_group_id) AS sg_store_count,
            (SELECT business_type_name FROM finascop_business_type WHERE business_type_id = (SELECT business_type_id FROM finascop_branch_group_business_type fb WHERE fb.store_group_id = a.store_group_id AND is_primary = 1 LIMIT 1)) AS sg_primary_bt,
            ...
            (SELECT br_Name FROM finascop_branch a WHERE br_isdefaultstore = 1 AND br_storeGroup = store_group_id) as defStrGrp,
            IF(store_group_grosmartMerchant = 1,'YES','NO') AS grosmartMerchant,store_group_grosmartMerchant FROM finascop_branch_group a 
        " . "{$search}{$searchitem}  ORDER BY {$sort} {$dir} limit $rec_start,$rec_limit";
    ...
    case 'storegroupsdetailsView':
        ...
        if ($store_group_id || $ID) {
            ...
            (SELECT GROUP_CONCAT(business_type_name) FROM finascop_business_type INNER JOIN finascop_branch_group_business_type  ON finascop_business_type.business_type_id = finascop_branch_group_business_type.business_type_id WHERE finascop_branch_group_business_type.store_group_id = a.store_group_id AND is_primary = 0) AS sg_additional_bt,
            (SELECT br_Name FROM finascop_branch a WHERE br_isdefaultstore = 1 AND br_storeGroup = store_group_id) as defStrGrp,IF(store_group_outofStock = 1,'YES','NO') AS store_group_outofStock,
            IF(store_group_grosmartMerchant = 1,'YES','NO') AS store_group_grosmartMerchant,contactNumber,
            IF(isFeatured = 1,'YES','NO') AS isFeatured FROM finascop_branch_group as a  WHERE store_group_id =" . $store_group_id, true);
            $data['success'] = true;
            echo json_encode($data);
        }
        break;
    ...
    case 'storegroups_form_load':
        ...
        $store_group_id = isset($_POST['store_group_id']) ? intval($_POST['store_group_id']) : 0;
        if ($store_group_id) {
            $sql = "SELECT  a.store_group_id,store_group_name,status AS comboMasterStoreGroupsStatus,store_group_grosmartMerchant,isFeatured,store_group_outofStock,(SELECT business_type_id FROM finascop_branch_group_business_type fb WHERE fb.store_group_id = a.store_group_id AND is_primary = 1) AS store_group_primary_businessType,"
                . "(SELECT GROUP_CONCAT(business_type_id) FROM finascop_branch_group_business_type fb WHERE fb.store_group_id = a.store_group_id AND is_primary = 0) AS store_group_additional_businessType,
                (SELECT GROUP_CONCAT(business_type_name) FROM finascop_business_type INNER JOIN finascop_branch_group_business_type  ON finascop_business_type.business_type_id = finascop_branch_group_business_type.business_type_id WHERE finascop_branch_group_business_type.store_group_id = a.store_group_id AND is_primary = 0) AS sg_additional_bt,
                contactNumber AS store_group_mobile,exclusiveMerchant,reservedMerchant,(SELECT GROUP_CONCAT(store_group_name ORDER BY FIND_IN_SET(b.store_group_id, a.exclusiveMerchant)) FROM finascop_branch_group b WHERE FIND_IN_SET(b.store_group_id, a.exclusiveMerchant)) AS exclusiveMerchantName,
                (SELECT GROUP_CONCAT(store_group_name ORDER BY FIND_IN_SET(b.store_group_id, a.reservedMerchant)) FROM finascop_branch_group b WHERE FIND_IN_SET(b.store_group_id, a.reservedMerchant)) AS reservedMerchantName
                FROM finascop_branch_group a WHERE a.store_group_id =" . $store_group_id;
            $results = $db->getFromDB($sql, true);
            //$merchantData = $sqlservdb->getRow("select st.Name,st.Package,st.DisplayName,st.StorePhone,st.StoreContactName,st.StoreAddress from AppTenant apt
//inner join store st on st.TenantId = apt.id where apt.StoreId = ?",[$store_group_id]);
            if (!$results) {
                echo '{"success":true,"data":[]}';
            } else {
    ...
    case 'saveStoreGroups':
        ...
        if ($status == 1) {
            $findata['name'] = $_POST['name'];
            $findata['mobile'] = $_POST['mobile'];
            $findata['refid'] = $data['storeRefId'];
            try {
                ...
                $url = DATAENTRY_ENDPOINT . "/api/CreateTenantLedger";
                curl_setopt($cURLConnection, CURLOPT_URL, $url);
                curl_setopt($cURLConnection, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($cURLConnection, CURLOPT_POST, 1);
                curl_setopt($cURLConnection, CURLOPT_HTTPHEADER, $headers);
                curl_setopt($cURLConnection, CURLOPT_POSTFIELDS, json_encode($findata));
                ...
                $log['type'] = "TenantCreation";
                $log['status'] = 1;
                $log['comments'] = "Result Exception :" . $ex;
    ...
    case 'listBusinessCategorys':
        ...
        $db->printGridJson($countQuery, $listQuery);
        break;
    case 'businesscategorys_form_load':

        $business_category_id = isset($_POST['business_category_id']) ? intval($_POST['business_category_id']) : 0;
        if ($business_category_id) {
            $sql = "SELECT  business_category_id,business_category_name,status AS comboMasterBusinessCategorysStatus,business_category_ingroup,rbc_business_type FROM retaline_business_category WHERE business_category_id =" . $business_category_id;
            $results = $db->getFromDB($sql, true);
            if (!$results) {
                echo '{"success":true,"data":[]}';
            } else {
        ...
        break;
    case 'businesscategorysdetailsView':
