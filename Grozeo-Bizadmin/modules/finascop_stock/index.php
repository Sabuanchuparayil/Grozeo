...
switch ($op) {
    ...
    case 'saveConversion':
        ...
        $ConsumptionData = json_decode(stripslashes($consumptiongridData), true);

        foreach ($ConsumptionData as $k => $v) {

            $item_id = $v['item_id'];
            $qty = $v['qty'];
            $qry = "SELECT stbr_id from " . FINASCOP_DB . "finascop_stock_branch WHERE {$item_id} = stit_ID AND br_Id = $currentBranch ";
        ...
        $producedgridData = json_decode(stripslashes($producedgridData), true);
    ...
    case 'getpartyData':
        ...
        if ($_SESSION['admin']->br_PyramidLevel == 1) {
            ...
            /*if ($_SESSION['admin']->IsSuperUser == 'Yes') {
                ...
                $filter_query .= " AND a.br_id = {$br_id} ";
            }*/
        } else {
            $filter_query .= " AND a.br_id = {$_SESSION['admin']->finascop_current_branch_id} ";
        }
        

        $qry = "select br_Name,stpa_id as customerId, stpa_Fname,stpa_Lname as stpa_Lname,stpa_Address as stpa_Address,stpa_ContactPerson as stpa_ContactPerson,stpa_MobileNo as stpa_MobileNo,stpa_Email as stpa_Email,stpa_PanNo as stpa_PanNo ,stpa_City as stpa_City,stpa_PINCODE as stpa_PINCODE,stpa_GSTIN as stpa_GSTIN,"
                ...
                deliverMode_stdDelivery,asctedbrach_stdDelivery,deliverMode_cpr,deliveryRule_courier,deliveryRule_express,deliveryRule_slotted,stpa_latitude,stpa_longitude,asctedbrach_cpr "
                . "  from   finascop_stock_party a INNER JOIN finascop_branch ON finascop_branch.br_ID = a.br_id where 1=1  and a.stpa_IsVendor=" . ( $_POST['partytype'] == 1 ? 1 : 0) . " and $filter_query order by stpa_id asc limit {$rec_start},{$rec_limit} ";

        $data = $db->getMultipleData($qry, true);

        $countQuery = "SELECT COUNT(*) FROM " . FINASCOP_DB . " finascop_stock_party a where 1=1 and a.stpa_IsVendor=" . ( $_POST['partytype'] == 1 ? 1 : 0) . " and $filter_query ";
        $count = $db->getItemFromDB($countQuery);
...
//        }
    ...
    case 'vendoritemlisting':
        ...
        $item_id = $_POST['current_type'];
         $cond = " WHERE 1=1 AND (directPurchase = 1  || stit_ParentItemId = 0) and stit_status = 1 ";
...
//        }
        ...
        if (customerId != '') {
            ...
            $qry = "SELECT stpi_id,stit_type,IF(stit_type = 1,'Medicine','Product') AS itemType,finascop_stock_itemmaster.stit_id AS itemId,stit_SKU AS itemName,stit_brand_name,stit_quantity,least_package_type_name 
                    FROM finascop_stock_party_items INNER JOIN finascop_stock_itemmaster ON finascop_stock_itemmaster .stit_id= finascop_stock_party_items.stit_id WHERE stpa_id='{$customerId}' and stit_status = 1 ORDER BY $rec_sort $rec_sort_dir";
