    ...
    case 'listGroups':
        ...
        if (isset($_POST['filter'])) {
            ...
            foreach ($_POST['filter'] as $key => $v) {
                $_allowedFilterFields = ['SlNo', 'group_ReferenceId', 'group_name', 'id', 'nature_of_group'];
                if (!in_array($v['field'], $_allowedFilterFields)) continue;
                $safeValue = $db->sanitizeString($v['data']['value']);
                switch ($v['field']) {

                    case 'group_name':
                        $filterCon .= $precondition . "GroupName like '" . $safeValue . "%'";
                        break;
                    case 'nature_of_group':
                        $filterCon .= $precondition . "NatGroupName like '" . $safeValue . "%'";
                        break;
                    case 'group_ReferenceId':
                        $filterCon .= $precondition . "Group_ReferenceId like '" . $safeValue . "%'";
                        break;
                }
    ...
    case 'getLedgers' :
        ...
        if (isset($filter)) {
           ...
           foreach ($_POST['filter'] as $key => $val) {
               $_allowedFilterFields = ['ledgertypename', 'ledger_ReferenceId', 'accled_Comp', 'GroupName'];
               if (!in_array($val['field'], $_allowedFilterFields)) continue;
               $safeValue = $db->sanitizeString($val['data']['value']);
                switch ($val['field']) {

                    case 'ledger_ReferenceId':
                        $filter_query  .= " AND al.accled_ReferenceId LIKE '%" . $safeValue . "%' ";
                        break;
                    default:
                        $filter_query .= " AND al." . $val['field'] . " LIKE '%" . $safeValue . "%' ";
                        break;
                }
