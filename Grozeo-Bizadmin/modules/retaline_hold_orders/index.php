...
switch ($op) {
    case 'listHoldOrders':
        ...
        $query = " SELECT order_id,order_order_id,order_packedbags_count,bco.order_customer_id,order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,DATE_FORMAT(created_at,'%d-%m-%Y') AS order_created_on,
...
//                $datas[$i]['quor_Type'] = $quor_Type;
    ...
    case 'getSatelliteBranch':
        ...
        $qry = "select br_ID,CONCAT(br_Name ,'-',branch_shortname) as br_Name from finascop_branch where br_status = 'Active' AND br_type = 1 " . $con . " order by br_Name ";
    ...
    case 'listDarkStoreJobs':
        ...
        if ($_SESSION['admin']->br_PyramidLevel == 1) {
            $brquery = " ";
        }else if ($_SESSION['admin']->br_PyramidLevel == 2) {
            $reatailors = $db->getItemFromDB("SELECT GROUP_CONCAT(br_ID) FROM finascop_branch WHERE br_type = 1 ");
            $current_branch_id = $reatailors;
            $brquery = " AND order_branch_id IN ({$current_branch_id} ";
        } else {
            $current_branch_id = $_SESSION['admin']->finascop_current_branch_id;
            $brquery = " AND order_branch_id IN ({$current_branch_id}";
        }
        ...
        $query = " SELECT order_id,order_order_id,order_packedbags_count,bco.order_customer_id,order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,DATE_FORMAT(created_at,'%d-%m-%Y') AS order_created_on,
             TIME_FORMAT(cast(created_at as time),'%r') as ordertime,admin_description AS order_status,admin_description,order_payment_gateway_refid,order_payment_gateway_refid_crc32,
            (SELECT cust_customer_name FROM `retaline_customer` WHERE cust_id = bco.order_customer_id) AS delivery_to,(SELECT cust_mobile FROM `retaline_customer` WHERE cust_id = bco.order_customer_id) AS cust_mobile,
            order_HasReturn,order_ItemsReturned,order_ReturnVerified,created_at,order_total_amount,payment_mode,order_method,br_parentPacking 
            FROM retaline_customer_order bco
                        INNER JOIN retaline_customer_order_status bcos ON bcos.status_id = bco.status_id  
                        INNER JOIN finascop_branch ON br_ID = order_branch_id  WHERE 1 = 1 and bco.status_id IN (40,41,42,43,44) {$brquery}) "; //and bco.status_id = 22 
        $countQuery = $db->getItemFromDB(" SELECT COUNT(*) FROM ({$query}) AS orerCount {$filter_qry} ORDER BY  {$sort} {$dir} ");
        $listQuery = "SELECT * FROM({$query}) as orderList  {$filter_qry}  ORDER BY  {$sort} {$dir} limit " . $start . "," . $limit;
        $datas = $db->getMulipleData($listQuery, true);
        $resCount = count($datas);
...
//                $datas[$i]['quor_Type'] = $quor_Type;
        ...
        break;
    case 'pushToStockRequest':
        $toRequestDatas = json_decode(stripslashes($_POST['gridData']));
        $datTime = date('Y-m-d H:i:s');

        $db->query('begin');
        $fsr['fsr_source'] = $_POST['satelliteBranch'];
        $fsr['fsr_destination'] = $db->getItemSafe("SELECT br_typeParent FROM finascop_branch WHERE br_ID = ?", "i", [$_POST['satelliteBranch']]);
        ...
        foreach ($toRequestDatas as $toRequestData) {
            $orderDeatils = $db->getMultipleData("SELECT item_product_id,item_order_qty FROM retaline_customer_order_items WHERE customer_order_id = {$toRequestData->order_id}", true);
            foreach ($orderDeatils as $orderDeatil) {
                $isItemEntry = $db->getItemFromDB("SELECT COUNT(*) FROM finascop_stock_request_details WHERE fsr_id = {$lastId} AND fsr_ItemId = {$orderDeatil['item_product_id']}");
                if ($isItemEntry > 0) {
                    $status = $db->query("UPDATE finascop_stock_request_details SET fsr_RequiredItemQty = fsr_RequiredItemQty + {$orderDeatil['item_order_qty']},"
                            . "fsr_ApprovedItemQty = fsr_ApprovedItemQty + {$orderDeatil['item_order_qty']},fsr_leastSKUCount = fsr_leastSKUCount + {$orderDeatil['item_order_qty']} WHERE fsr_id = {$lastId} AND fsr_ItemId = {$orderDeatil['item_product_id']} ");
                } else {
                    $fsrd['fsrd_status'] = 1;
                    $fsrd['fsr_id'] = $lastId;
                    $fsrd['fsrd_createdOn'] = date("Y-m-d H:i");
                    $fsrd['fsrd_createdBy'] = $_SESSION['admin']->UserId;
                    ...
                    $fsrd['fsr_ItemMRP'] = $db->getItemFromDB("SELECT fpod_leastSKUmrp FROM finascop_stock_branch_inventory WHERE stit_ID = {$orderDeatil['item_product_id']} AND branch_id = {$fsr['fsr_destination']} ");
            ...
            $data['fsr_id'] = $lastId;
            $data['order_id'] = $toRequestData->order_id;
            ...
            if ($status = 1) {
                ...
                $status = $db->perform('retaline_customer_order', $rcodata, 'update', " status_id = 40 AND order_id = {$toRequestData->order_id}");
            }
        }
        $status = $db->query('commit');
        if ($status == 1) {
