...
switch ($op) {
    ...
    case 'saveStockDetails':
        ...
        $fsbiu['fbiu_createdOn'] = date("Y-m-d H:i:s");
        $db->query('begin');
        $isEntry = $db->getItemSafe("SELECT fbiu_id FROM finascop_stock_branch_inventory_upload WHERE fbiu_uploadedapikey = '?'", "i", [$_POST['uuid']]);
        if ($isEntry > 0) {
            $lastId = $isEntry;
        } else {
            $status = $db->perform('finascop_stock_branch_inventory_upload', $fsbiu);
    ...
    case 'cofirmStockUpdate':
        $uuid = $_POST['uuid'];
        $fbiu_id = $db->getItemSafe("SELECT fbiu_id FROM finascop_stock_branch_inventory_upload WHERE fbiu_uploadedapikey = '?'", "i", [$_POST['uuid']]);
        if ($fbiu_id > 0) {
            $fbiud['fbiu_status'] = 1;
