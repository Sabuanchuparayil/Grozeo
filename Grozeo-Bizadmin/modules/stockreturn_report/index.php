...
class stockReturnReportGeneration {
    ...
    public function _getReturnDetailsReportGridData($postvar) {
        ...
        $countQuery = "SELECT COUNT(1) FROM finascop_stock_item_inventorydetails fi "
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_stock_item_inventorydetails_status fs ON fs.stiid_status = fi.stiid_status INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$searchitem}";
        $listQuery = "SELECT stiid_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,fpo_id,fpo_vendorName,fpo_poNumber,DATE_FORMAT(fpo_poDate,'%d-%m-%Y') AS fpo_poDate,stiid_barcode,stiid_itemmastername,"
                ...
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_stock_item_inventorydetails_status fs ON fs.stiid_status = fi.stiid_status INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$searchitem} ORDER BY {$sort} {$dir} LIMIT $start,$limit ";
...
//        $sort = ($sort=='order_created_on') ? 'DATE(order_created_on)' : $sort;
        ...
        $query = " SELECT rtrqo_id,order_order_id,order_packedbags_count,order_customer_id,order_branch_id,CONCAT(br_Name ,'-',branch_shortname) as br_Name,bco.status_id as status,DATE_FORMAT(created_at,'%d-%m-%Y') AS order_created_on,TIME_FORMAT(cast(created_at as time),'%r') as ordertime,admin_description AS order_status,
