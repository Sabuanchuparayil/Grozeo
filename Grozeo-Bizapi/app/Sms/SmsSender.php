...
class SmsSender {
    ...
    public function __construct(SmsEmailRepository $smsEmailLogs, TextLocalSms $curlurl, OtpController $genrtOtp) {
        $this->smsEmailLogs = $smsEmailLogs;
        $this->curlurl = $curlurl;
        $this->genrtOtp = $genrtOtp;
    }
    ...
    public function fetchContentSendSms($templatedata, $mobile, $templateType, $strGrpId = -1, $strGrpType = 1)
    {
        try
        {
            ...
            $getTemplateContent = DB::select('SELECT templateContent,templateId,templateHeader, (SELECT COUNT(*) AS ismobile FROM test_mobile WHERE mobile=?) AS ismobile FROM sms_templates WHERE stn_templateNameId= ? AND (store_group_id = ? OR store_group_id = 0) AND is_used = ? ORDER BY store_group_id DESC LIMIT 1', [$mobile, $templateType, $strGrpId, $strGrpType]);
