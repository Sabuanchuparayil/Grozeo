...
class TextLocalSms implements SmsProviderInterFace {
    ...
    public function sendSms($mobile, $msg, $templid = 0) {
        ...
        if (config('sms.pocketkart_sms_provider') == 'true') {
            $url = "http://sms.bulksmsind.in/sendSMS?username=" . config('sms.bulksms_username', '') . "&message=" . urlencode($msg) . "&sendername=PTKART&smstype=TRANS&numbers=" . $mobile . "&apikey=" . config('sms.bulksms_api_key', '');
