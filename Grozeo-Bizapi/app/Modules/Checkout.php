...
class Checkout 
{
   ...
   // public function __construct(InterfacePayment $paymenty)
    ...
    public function confirmorder($request)
    {
        ...
        $orders = Order::where(function($query) use ($podToOnline, $orderGroupId, $orderId){
                    $query->where([
        ...
        if (isset($request['use_wallet']) && $request['use_wallet'] == 1) {
            ...
            foreach ($orders as $order) {
                if($order->status_id < 5)
                {
                    ...
                    // if 0 wallet balance then skip processing wallet.
                    if($walletAmount <= 0)
                    {
                        ...
                        if($checkWalletTransaction)
                        {
                            
                            $walletAmount = abs($checkWalletTransaction->brcw_Amount);
                        }
                        else
                        {
    ...
    public function storeWalletTransaction($order_id, $order_order_id, $branch_id, $usedWalletAmount)
    {
        ...
        Customer::where('cust_id',auth_user()->cust_id)->update([
            'cust_walletbalance' => (($newBalance >= 0) ? $newBalance : 0),
        ]);
    ...
    public function podToOnlineGenerateLink($orderId)
    {
        ...
        if($order->first()->payment_mode == 1)
        {
            return $this->onlinePayment($order->get(), [], @$order->first()->order_wallet_amount, 0, 1);
        }
        return null;
    }
...
//        }
    ...
    private function updateOrderDetails($order,$status, $paymentMode, $walletAmount = 0, $isFullyWallet = false){
        ...
        $amountPayable = ($paymentMode == 5) ? 0 : $total - $walletAmount;
        $updatedOrder = Order::where('order_id', $order->order_id)->where('status_id','<=',4)->update([
            'status_id'                     => $status,
     ...
     public function getCurrntOrderList($list)
    {
		...
		//if(!(isset($value) && array_key_exists('order_id', $value)))
            ...
            $domain = "https://" . config('filesystems.disks.s3.bucket') . "." . config('filesystems.disks.s3.driver') . ".".config('filesystems.disks.s3.region', 'ap-southeast-1').".amazonaws.com/products/";
