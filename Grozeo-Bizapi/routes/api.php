...
Route::group(['prefix' => 'auth', 'middleware' => 'jwt.verify'], function () {
    Route::post('token/refresh', [\App\Http\Controllers\Auth\TokenController::class, 'refresh']);
    Route::get('me', [\App\Http\Controllers\Auth\TokenController::class, 'me']);
});

Route::get('health', [\App\Http\Controllers\HealthController::class, 'ping']);

Route::group(['middleware' => ['jwt.verify', 'role:superadmin']], function () {
    Route::get('admin/health', [\App\Http\Controllers\HealthController::class, 'check']);
});
...
Route::group(['prefix' => 'signup'], function () {
    ...
    Route::post('socials/{type}', [SocialLoginController::class, 'socialLogins'])->middleware('throttle:5,1');
    Route::post('verify/password', [VerificationController::class, 'verifyPassword'])->middleware('throttle:3,1');
    Route::post('verify/{type?}', [VerificationController::class, 'verify'])->middleware('throttle:3,1');
    Route::post('pincode', [PincodeController::class, 'get']);
    Route::post('customer', [RegistrationController::class, 'store'])->middleware('throttle:5,1');
    Route::post('{type}', [VerificationController::class, 'store'])->middleware('throttle:3,1');
});
...
Route::get('initial', 'HomeScreenController@getCredentials');
...
Route::middleware('auth.guest')->group(function () {
    Route::post('es/key/search', 'ProductSearchController@newESSearch');
    Route::post('es/product/search', 'ProductSearchController@newESSortFiletrSearch');
});
//=====================================
