...
use App\Models\TransferOrderDetails;
...
class B2CToTransferOrder
{
    ...
    private function insert($order_id)
    {
        ...
        {
            $brSettings = $branchData->settings->toArray();
            $checkTPPacking = array_values(
                array_filter($brSettings, function($item) {
                    return ($item['type'] == 1 && $item['tp_type'] == 'status' && $item['tp_value'] == '1');
                })
            );
            $checkTPPacking = (!empty($checkTPPacking)) ? array_values($checkTPPacking) : [];
            if(@$checkTPPacking[0]['id'])
            {
                $transferOrder->fsto_status = 23;
            }
        }
