...
namespace Retaline.Core.Services.HelperServices
{
    ...
    public class HttpHelperService : IHttpHelperService
    {
        ...
        private void SaveGuestUser(GuestData guestUser)
        {
            ...
            _httpContextAccessor.HttpContext.Session.SetString(GuestCookieName, encryptedData);
            //_httpContextAccessor.HttpContext.Response.Cookies.Append(GuestCookieName, encryptedData, new CookieOptions
            //{
            //    IsEssential = true,
            //    HttpOnly = true,
            //    Secure = true,
        ...
        // Decrypt Guest User Object
        private GuestData DecryptGuestUser(string encryptedData)
        {
            var serializedData = Decrypt(encryptedData);
        ...
        private GuestData CreateNewGuestUser()
        {
            ...
            try
            {
                ...
                {
                    httpClient.Dispose();
                    var data = response.Content.ReadFromJsonAsync<GuestRoot>(new JsonSerializerOptions() { Encoder = System.Text.Encodings.Web.JavaScriptEncoder.UnsafeRelaxedJsonEscaping }).GetAwaiter().GetResult();
        ...
        // Encrypt Method (AES Encryption Example)
        private string Encrypt(string plainText)
        {
            // Use a secure encryption mechanism to protect the cookie data
            // For example, AES encryption with a shared key
        ...
        {
            // Decrypt using the same encryption mechanism
