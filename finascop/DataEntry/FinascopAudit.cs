            ...
            if (newBranches != null && newBranches.Rows.Count > 0)
            {
                ...
                foreach (DataRow newBranch in newBranches.Rows)
                {
                    ...
                    if (String.IsNullOrEmpty(storeRefId))
                    {
                        ...
                        string sqlLedgerrRefId = $"SELECT refId FROM [ledger] WHERE name = @ledgeName";
                        ...
                        var ledId = DataService.ExecuteScalar(sqlLogEntryCheck, parmeters: lecprms);
                        string ledgerId = System.Convert.ToString(ledId);
                        string safeMobile = ledger_Name.Contains("_") ? ledger_Name.Substring(ledger_Name.IndexOf("_") + 1) : "";
                        var data = new { name = newBranch.Field<string>("store_group_name"), mobile = safeMobile, refid = ledgerRefId };
