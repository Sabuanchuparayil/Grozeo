...
switch ($op) {
    ...
    case 'cate_form_load':
        ...
        if ($category_id) {
            $sql = "SELECT category_id,category_name AS cat_name,isHome as mc_isHome,isInCategory as mc_isInCategory,parent_category,(SELECT parent_category_businessType FROM mypha_productparent_category WHERE parent_category_id = mypha_productcategory.parent_category) as primary_businessType,(SELECT parent_category FROM mypha_productparent_category WHERE parent_category_id = mypha_productcategory.parent_category) as parent_categoryname,status as statuscat FROM mypha_productcategory  WHERE category_id = " . $category_id;
    ...
    case 'CatdetailsView':
        ...
        if ($category_id || $ID) {
            ...
            $data = $db->getFromDB("SELECT category_id,category_name AS cat_name,IF((isHome=1),'Yes','No') AS mc_isHome,IF((isInCategory=1),'Yes','No') AS mc_isInCategory,parent_category,status,IF(image_url IS NOT NULL,'present','nil') AS image,"
                . "(SELECT parent_category FROM mypha_productparent_category WHERE parent_category_id = mc.parent_category) as parent_category_name,image_url FROM mypha_productcategory mc WHERE category_id =" . $category_id, true);
            $data['success'] = true;
    ...
    case 'saveCategory':
        ...
        if (empty($_POST['id'])) {
            ...
            $CategoryUnique = $db->getItemFromDB("SELECT COUNT(*) from mypha_productcategory WHERE category_name ='{$category_name}'  AND parent_category = {$parent_category}");
            if ($CategoryUnique > 0) {
                ...
                $status = $parentdb->perform('mypha_productcategory', $data);
            }
        } else {
            ...
            $CategoryUnique = $db->getItemFromDB("SELECT COUNT(*) from mypha_productcategory WHERE category_name ='{$category_name}' AND parent_category = {$parent_category} AND category_id!='{$category_id}' ");
            if ($CategoryUnique > 0) {
                echo "{success: false, message:'This  Category already existing.'}";
                exit;
            } else {
                ...
                $status = $parentdb->perform("mypha_productcategory", $data, 'update', 'category_id =' . $data['category_id']);
            }
        }
    ...
    case 'saveParentCategory':
        ...
        if (empty($_POST['id'])) {
            ...
            $parentCategoryUnique = $db->getItemSafe("SELECT COUNT(*) from mypha_productparent_category WHERE parent_category ='{$parent_category}'  and parent_category_businessType = ?", "s", [$_POST['parent_category_businessType']]);
            if ($parentCategoryUnique > 0) {
                ...
                $data['parent_category_id'] = $lastId;
                $status = $parentdb->perform('mypha_productparent_category', $data);
            }
        } else {
            ...
            $parentCategoryUnique = $db->getItemSafe("SELECT COUNT(*) from mypha_productparent_category WHERE parent_category = '{$parent_category}' AND parent_category_businessType ='?' AND parent_category_id <> {$data['parent_category_id']}", "s", [$_POST['parent_category_businessType']]);
            if ($parentCategoryUnique > 0) {
                ...
                $status = $parentdb->perform("mypha_productparent_category", $data, 'update', 'parent_category_id =' . $data['parent_category_id']);
    ...
    case 'listSubcategory':
        $limit = isset($_POST['limit']) ? $_POST['limit'] : 19;
        $start = isset($_POST['start']) ? $_POST['start'] : 0;
        ...
        IF((isCompositionDelator=1),'Yes','No')AS sbc_isCompositionDelator,IF((isNonGstRetailer=1),'Yes','No') AS sbc_isNonGstRetailer,IF((isPerishable=1),'Yes','No') AS sbc_isPerishable,IF((hasRestaurantService=1),'Yes','No') AS sbc_hasRestaurantService,processingTime,"
    ...
    case 'subCatdetailsView':
        ...
        if ($sub_category_id || $ID) {
            ...
            IF((hasAgeVerification=1),'Yes','No') AS sbc_hasAgeVerification,"
                . "(SELECT category_name FROM mypha_productcategory WHERE category_id = main_category) AS main_categoryName,(SELECT parent_category FROM mypha_productcategory WHERE category_id = main_category) AS parent_categorysc,packingMode,
                ...
                CASE WHEN packingMode = 0 THEN 'Group Packing' WHEN packingMode = 1 THEN 'Pack the items independently' WHEN packingMode = 2 THEN 'Pack same items together'  END AS  packingModeName
                 FROM mypha_productsubcategory WHERE sub_category_id =" . $sub_category_id, true);
            $data['success'] = true;
            echo json_encode($data);
        }
        break;
    case 'subcate_form_load':
        ...
        if ($sub_category_id) {
            ...
            hasAgeVerification AS sbc_hasAgeVerification,main_category,(SELECT category_name FROM mypha_productcategory WHERE category_id = main_category) AS main_categoryName,packingMode AS radiobuttonPackingMode,
            (SELECT parent_category FROM mypha_productcategory WHERE category_id = main_category) AS parent_categorysc,(SELECT parent_category FROM mypha_productparent_category WHERE parent_category_id = (SELECT parent_category FROM mypha_productcategory WHERE category_id = main_category)) AS parent_categoryname,
            (SELECT parent_category_businessType FROM mypha_productparent_category WHERE parent_category_id = (SELECT parent_category FROM mypha_productcategory WHERE category_id = main_category)) AS primary_businessTypesc,(SELECT business_type_name FROM finascop_business_type WHERE business_type_id = (SELECT parent_category_businessType FROM mypha_productparent_category WHERE parent_category_id = (SELECT parent_category FROM mypha_productcategory WHERE category_id = main_category))) AS btName,processingTime as sbc_processingTime,
            status
            FROM mypha_productsubcategory  WHERE sub_category_id = " . $sub_category_id;
            $results = $db->getFromDB($sql, true);
            if (!$results) {
                echo '{"success":true,"data":[]}';
            } else {
    ...
    case 'saveSubcategory':
        ...
        if ($data['sub_category_id'] > 0) {
            ...
            if ($SubCategoryUnique > 0) {
                ...
                $status = $parentdb->perform("mypha_productsubcategory", $data, 'update', 'sub_category_id =' . $data['sub_category_id']);
                $status = $parentdb->perform('finascop_stock_itemmaster', $fsim, 'update', " product_category = {$lastId} AND isMedicine = 0");
                $status = $parentdb->perform('finascop_stock_uniqueitem', $fsui, 'update', " fsi_category_id = {$lastId} AND isMedicine = 0");
            }
        } else {
            ...
            if ($SubCategoryUnique > 0) {
                ...
                $data['sub_category_id'] = $lastId;
                unset($data['isVerified']);
                $status = $parentdb->perform('mypha_productsubcategory', $data);
            }
        }

        $return_rec = $db->getFromDb("SELECT sub_category_id,sub_category,main_category,status,
        (SELECT category_name FROM mypha_productcategory mc WHERE category_id = main_category) as main_category_name FROM mypha_productsubcategory WHERE sub_category_id = {$lastId}", true);
    ...
    case 'hns_form_load':
        ...
        $hsn_id = isset($_POST['hsn_id']) ? intval($_POST['hsn_id']) : 0;
        if ($hsn_id) {
            $sql = "SELECT id as hsn_id_pk,hsn_id,hsn_code as oldhsn_code ,hsn_code,hsnGst,hsnCess,hsn_description,status AS comboMasterHSNStatus FROM hsn_value 
    ...
    case 'saveHSN':
        ...
        $id = $_POST['id'];
        if ($_POST['hsn_id'] > 0)
            $mappingId = $db->getItemSafe("SELECT mappingId FROM finascop_hsn WHERE hsn_id = ?", "i", [$_POST['hsn_id']]);
        $data = array(
            "hsnGst" => $_POST['gst'],
            "hsnCess" => $_POST['cess'],
        ...
        $hsn_code = $_POST['code'];
        $hsn_code = str_replace(' ', '', $hsn_code);
        $oldhsn_code = $_POST['oldhsn_code'];
        $oldhsn_code = str_replace(' ', '', $oldhsn_code);
        $hsnGst = $data['hsnGst'];
        $status = $_POST['status'];
        $mdata['hsn_description'] = $description;

        if ($id > 0 && ($hsn_code != $oldhsn_code)) {
            echo "{'success':false,'valid':false,'message': 'Mismatch in HSN proceed with create.'}";
            exit();
        }
        if ($mappingId > 0)
            $ghsn_id = $mappingId;
        else
            $ghsn_id = $parentdb->getItemFromDB("SELECT hsn_id FROM finascop_hsn WHERE REPLACE(hsn_code, ' ', '') ='{$hsn_code}'");
        $ghsnValueId = $parentdb->getItemFromDB("SELECT id from hsn_value WHERE hsnGst = {$_POST['gst']} AND  hsnId = '{$ghsn_id}' ");
        if ($ghsn_id > 0) {
            $mdata['hsn_code'] =  $hsn_code;
            $mdata['updated_on'] = date('Y-m-d H:i:s');
            $mdata['updated_by'] = $userid;
            $mdata['status'] = $status;
            $status = $parentdb->perform('finascop_hsn', $mdata, 'update', " hsn_id = {$ghsn_id}");
            $data['hsnId'] = $ghsn_id;
        } else {
            ...
            $mdata['hsn_code'] =  $hsn_code;
            $mdata['status'] =  $status;
            $status = $parentdb->perform('finascop_hsn', $mdata);
            $ghsn_id = $parentdb->insert_id();
            $data['hsnId'] = $ghsn_id;
        }
        if ($ghsnValueId > 0) {
            $status = $parentdb->perform("hsn_value", $data, 'update', 'id =' . $ghsnValueId);
        } else {
            $status = $parentdb->perform("hsn_value", $data);
        }

        $mdata['mappingId'] = $ghsn_id;
        if ($_POST['hsn_id'] > 0)
            $hsn_id = $_POST['hsn_id'];
        else
            $hsn_id = $db->getItemFromDB("SELECT hsn_id FROM finascop_hsn WHERE REPLACE(hsn_code, ' ', '') ='{$hsn_code}'");
        $hsnValueId = $db->getItemSafe("SELECT id from hsn_value WHERE hsnGst = ? AND  hsnId = '{$hsn_id}' ", "s", [$_POST['gst']]);
        if ($hsn_id > 0) {
            $mdata['hsn_code'] =  $hsn_code;
            $mdata['updated_on'] = date('Y-m-d H:i:s');
            $mdata['updated_by'] = $userid;
        ...
        if ($hsnValueId > 0) {
            $status = $db->perform("hsn_value", $data, 'update', 'id =' . $hsnValueId);
        } else {
            $status = $db->perform("hsn_value", $data);
            $hsnValueId = $db->insert_id();
        }
        ...
        $itemCount = $db->getItemFromDB("SELECT COUNT(*) FROM finascop_stock_itemmaster WHERE stit_hsnId = {$hsn_id} AND taxValueId = {$hsnValueId}");
        if ($itemCount > 0) {
            $fsim['stit_HSN_code'] = $hsn_code;
            $fsim['stit_HSNCode'] = $hsn_code;
            $fsim['stit_GST'] = $hsnGst;
            $status = $db->perform('finascop_stock_itemmaster', $fsim, 'update', " stit_hsnId = {$hsn_id} AND taxValueId = {$hsnValueId} ");
            $status = $parentdb->perform('finascop_stock_itemmaster', $fsim, 'update', " stit_hsnId = {$hsn_id} AND taxValueId = {$hsnValueId} ");
        }
        ...
        INNER JOIN finascop_hsn ON hsnId = hsn_id WHERE id = {$hsnValueId}", true);
    ...
    case 'saveParentCategoryImage':
        ...
        $res = $parentdb->perform('mypha_productparent_category', $data, 'update', 'parent_category_id=' . $parent_category_id);
        $status = $db->query('commit');
    ...
    case 'saveSubCategoryImage':
        ...
        $status = $db->perform('mypha_productsubcategory', $data, 'update', 'sub_category_id=' . $subcategory_id);
        $status = $parentdb->perform("mypha_productsubcategory", $data, 'update', 'sub_category_id =' . $subcategory_id);
        $status = $db->query('commit');
    ...
    case 'saveMainCategoryImage':
        ...
        $status = $db->perform('mypha_productcategory', $data, 'update', 'category_id=' . $category_id);
        $status = $parentdb->perform('mypha_productcategory', $data, 'update', 'category_id=' . $category_id);
        $status = $db->query('commit');
    ...
    case 'saveBusinessTypes':
        ...
        if ($data['business_type_id'] > 0) {
            ...
            if ($packagenameUnique > 0) {
                ...
                $status = $parentdb->perform("finascop_business_type", $data, 'update', 'business_type_id =' . $data['business_type_id']);
            ...
            if ($packagenameUnique > 0) {
                ...
                $data['business_type_id'] = $lastId;
                $status = $parentdb->perform('finascop_business_type', $data);
            }
        }
    ...
    case 'getCategory':
        ...
        echo '{"success":true,"data":' . json_encode($data) . '}';
        break;
    case 'listverifySubCategory':
        $limit = isset($_POST['limit']) ? $_POST['limit'] : 12;
        $start = isset($_POST['start']) ? $_POST['start'] : 0;
        $sort = empty($sort) ? 'sub_category_id' : $sort;
        $dir = empty($dir) ? 'DESC' : $dir;
        $search = " WHERE 1=1 AND isVerified = 0";
        if (isset($data['filter'])) {
            $filter_qry .= buildSafeFilterQuery($data['filter'], $allowedFields, $db);
        }
        ...
        $countQuery = "SELECT COUNT(*) FROM mypha_productsubcategory sub 
            INNER JOIN mypha_productcategory mc ON mc.category_id = sub.main_category 
            INNER JOIN mypha_productparent_category ON mypha_productparent_category.parent_category_id = mc.parent_category 
            INNER JOIN finascop_business_type ON business_type_id = parent_category_businessType {$search}";
        $listQuery = "SELECT business_type_name,mypha_productparent_category.parent_category AS parent_category,category_name,sub_category_id,sub_category AS sub_cat,(sub.status),IF((sub.isHome=1),'Yes','No') AS isHome,IF((sub.isInCategory=1),'Yes','No') AS isInCategory,
