...
if (isset($_POST['filter'])) {
    ...
    foreach ($_POST['filter'] as $key => $val) {
        if (!array_key_exists($val['field'], $arr)) continue;
        $safeValue = $db->sanitizeString($val['data']['value']);
        $filter_part .= " and " . $arr[$val['field']] . " LIKE '%" . $safeValue . "%' ";
    }
}
$o = new stdClass();
...
$db->query('set @cnt=0');
$qry = "select * from (select @cnt:=@cnt+1 as rownum,sel.* from (select u.UserId as uidnr_admin, u.UserName as admin_username,u.UserEmail as admin_email, u.IsActive as admin_active,r.RoleName as admin_role_name,if(u.IsActive<>'Yes', 'user_disabled', 'user_enabled') as status_icon,CONCAT(p.FirstName, ' ', p.LastName) AS admin_fullname from " . FINASCOP_DB . "finascop_usr_master u inner join  " . FINASCOP_DB . "finascop_usr_profile p on u.UserId = p.UserId,sys_role r where u.RoleId=r.RoleId and IsSuperUser='No' and u.UserId <> '$CurrentUser' $filter_part order by $rec_sort $rec_sort_dir ) as sel) as sel2 limit $rec_start,$rec_limit";
