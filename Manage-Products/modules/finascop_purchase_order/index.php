...
//       exit();
        ...
        if ($adhocCount > 0) {
            $updatedDate = $db->getItemSafe("SELECT adhoc_updatedon FROM finascop_purchase_order_poadhoc WHERE adhoc_uniqueid = '?'", "i", [$_POST['fpot_uniqueid']]);
...
//        $data['bmd_customer'] = $bmdDetails['bmd_customer'];
        ...
        $newupdatedDate = $db->getItemSafe("SELECT adhoc_updatedon FROM finascop_purchase_order_poadhoc WHERE adhoc_uniqueid = '?'", "i", [$_POST['fpot_uniqueid']]);
...
//        }
        ...
        $listQuery = "SELECT  fpo_id,fpo_poNumber,DATE_FORMAT(fpo_poDate,'%d-%m-%Y') as fpo_poDate,fpo_paymentTerms,fpo_poDeliveryDate,fpo_poDeliveryType,fpo_paymentValue ,UserName,fpo_vendorName,
            DATE_FORMAT(fpo_validDate,'%d-%m-%Y') as fpo_validDate,IF((fpo_Active=1),'Active','Inactive') AS fpo_Active,IF((fpo_potype=2),'Initiated','Manual') AS fpo_potype   
 FROM finascop_purchase_order fp INNER JOIN finascop_usr_master um ON fp.fpo_poOrderedby = um.UserId {$filter_qry} ORDER BY {$sort} {$dir} LIMIT $start,$limit ";
    ...
    case 'vendorItemStore':
        ...
        $qry = "select stit_ID,stit_SKU from " . FINASCOP_DB . "finascop_stock_itemmaster where stit_ID IN({$itemIds}) order by stit_SKU";
        $data = $db->getMultipleData($qry, true);
        if (!empty($data)) {
...
//fpot_netamountet,fpot_pogstAmt
    ...
    case 'getCentralStores':
        $centralStore = $db->getMulipleData("SELECT br_ID,br_Name FROM finascop_branch WHERE br_cpd > 0 AND br_PyramidLevel <> 1 AND br_status = 'Active' ORDER BY br_PyramidLevel ASC", true);
...
//        }
 ...
 FROM finascop_purchase_order_poadhoc fp  {$filter_qry} where adhoc_uniqueid = '?'", "i", [$_POST['uniqueid']], true);
        if (!empty($podata)) {
...
//            }
    ...
    case 'checkAssignPO':
        ...
        if ($fpot_assigneddby > 0) {
            ...
            $status = $db->executeSafe("UPDATE finascop_purchase_order_temp SET fpot_assigneddby = {$_SESSION['admin']->Finascop_UserId} where fpot_uniqueid = '?'", "i", [$_POST['uniqueid']]);
