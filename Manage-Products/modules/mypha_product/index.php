 ...
 from " . FINASCOP_DB . "finascop_stock_itemmaster  where stit_ID = '$id' ";
    ...
    WHERE stit_product_variant = '" . mysqli_real_escape_string($db->linker(), $_POST['stit_product_variant']) . "' "
        ...
        if (!empty($_POST['stit_itemBarcode'])) {
            ...
            $isGlobal = $db->getItemSafe("SELECT COUNT(*) FROM finascop_stock_itemmaster_product_codes WHERE fsipc_stit_id = '{$fstrd['fsipc_stit_id']}' AND fsipc_isCompany = 1 AND fsipc_code = '?'", "i", [$_POST['stit_itemBarcode']]);
            $isGlobalCodeExist = $db->getItemSafe("SELECT COUNT(*) FROM finascop_stock_itemmaster_product_codes WHERE fsipc_stit_id <> '{$fstrd['fsipc_stit_id']}' AND fsipc_isCompany = 1 AND fsipc_code = '?'", "i", [$_POST['stit_itemBarcode']]);
