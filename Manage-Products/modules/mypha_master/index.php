...
switch ($op) {
    ...
    case 'saveUnit':
        ...
        if ($data['unit_id'] > 0) {
            ...
            if ($medCatUnique > 0) {
                ...
                $status = $parentdb->perform("mypha_unit", $data, 'update', 'unit_id =' . $data['unit_id']);
            }
        } else {
            ...
            if ($medCatUnique > 0) {
                ...
                $data['unit_id'] = $lastId;
                $status = $parentdb->perform('mypha_unit', $data);
            }
        }
    ...
    case 'listUnitValues':
        ...
        break;
    case 'saveUnitValue':
        $unitValueId = $_POST['unitValueId'];
        $unitId = $_POST['unitId'];
        $data['value'] = $_POST['unitValue'];
        $data['unitId'] = $_POST['unitId'];
        if ($unitValueId > 0) {
            $status = $db->perform('unit_value', $data,'update'," id = {$unitValueId}");
            $lastId = $db->insert_id();
        } else {
            ...
            if ($valueId == 0) {
                ...
                $data['id'] = $lastId;
                $status = $parentdb->perform("unit_value", $data);
            } else {
                $status = $db->perform('unit_value', $data, 'update', " unitId ='{$_POST['unitId']}' AND value = '{$_POST['unitValue']}' ");
                $status = $parentdb->perform('unit_value', $data, 'update', " unitId ='{$_POST['unitId']}' AND value = '{$_POST['unitValue']}' ");
            }
        }
