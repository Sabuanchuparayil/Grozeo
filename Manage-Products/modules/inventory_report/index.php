...
//        $sort = ($sort=='order_created_on') ? 'DATE(order_created_on)' : $sort;
        ...
        $query = " SELECT order_id,order_order_id,order_packedbags_count,order_customer_id,order_branch_id,br_Name,bco.status_id as status,DATE_FORMAT(created_at,'%d-%m-%Y') AS order_created_on,
    ...
    public function _getStockDetailsReportGridData($postvar) {
        ...
        $countQuery = "SELECT COUNT(1) FROM finascop_stock_item_inventorydetails "
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$where}{$searchitem}";
        $listQuery = "SELECT stiid_id,br_Name,fpo_id,fpo_vendorName,fpo_poNumber,DATE_FORMAT(fpo_poDate,'%d-%m-%Y') AS fpo_poDate,stiid_barcode,stiid_itemmastername,stiid_leastSKUmrp,"
                . "DATE_FORMAT(stiid_expirydate,'%d-%m-%Y') AS stiid_expirydate,stiid_batchno,cpd_branch_id,br_Name FROM finascop_stock_item_inventorydetails "
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$where}{$searchitem} ORDER BY {$sort} {$dir} LIMIT $start,$limit ";
    ...
    public function _getB2CSalesDetailsReportGridData($postvar) {
        ...
        if (!empty($_POST['search_stock_from_date']) && !empty($_POST['search_stock_to_date'])) {
            $searchitem .= " and DATE_FORMAT(created_at, '%Y-%m-%d') BETWEEN '" . date('Y-m-d', strtotime($_POST['search_stock_from_date'])) . "' AND '" . date('Y-m-d', strtotime($_POST['search_stock_to_date'])) . "'";
        }


        $countQuery = "SELECT COUNT(1) FROM retaline_customer_order
...
INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid 
INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$where}{$searchitem}";
        $listQuery = "SELECT order_order_id,finascop_stock_item_inventorydetails.stiid_id,br_Name,fpo_id,fpo_vendorName,fpo_poNumber,DATE_FORMAT(fpo_poDate,'%d-%m-%Y') AS fpo_poDate,finascop_stock_item_inventorydetails.stiid_barcode,stiid_itemmastername,stiid_leastSKUmrp,
DATE_FORMAT(stiid_expirydate,'%d-%m-%Y') AS stiid_expirydate,stiid_batchno,cpd_branch_id,br_Name,DATE_FORMAT(created_at,'%d-%m-%Y') AS created_at,CASE WHEN payment_mode = 1 THEN 'Pay On Delivery' WHEN payment_mode = 2 THEN 'Online Payment' WHEN payment_mode = 3 THEN 'Wallet' WHEN payment_mode = 4 THEN 'COD With Wallet' 
            WHEN payment_mode = 5 THEN 'Online With Wallet' WHEN payment_mode = 6 THEN 'Online On Delivery' WHEN payment_mode = 7 THEN 'Cash On Delivery' END AS payMode FROM retaline_customer_order
INNER JOIN finascop_stock_transfer_order ON fstr_id = order_id AND fsto_ordertype = 1 
INNER JOIN finascop_stock_transfer_order_details_barcodes ON finascop_stock_transfer_order_details_barcodes.fsto_id = finascop_stock_transfer_order.fsto_id 
INNER JOIN finascop_stock_item_inventorydetails ON finascop_stock_item_inventorydetails.stiid_id = finascop_stock_transfer_order_details_barcodes.stiid_id
INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid 
INNER JOIN finascop_branch ON br_ID = cpd_branch_id {$search}{$where}{$searchitem} ORDER BY {$sort} {$dir} LIMIT $start,$limit ";
    ...
    public function _getB2BSalesDetailsReportGridData($postvar) {
        ...
        $countQuery = "SELECT COUNT(1) FROM retaline_B2B_SalesOrder INNER JOIN finascop_stock_transfer_order ON fstr_id = bbso_id AND fsto_ordertype = 2 "
                . "INNER JOIN finascop_stock_transfer_order_details_barcodes ON finascop_stock_transfer_order_details_barcodes.fsto_id = finascop_stock_transfer_order.fsto_id "
                . "INNER JOIN finascop_stock_item_inventorydetails ON finascop_stock_item_inventorydetails.stiid_id = finascop_stock_transfer_order_details_barcodes.stiid_id "
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_branch ON finascop_branch.br_ID = cpd_branch_id {$search}{$where}{$searchitem}";
        $listQuery = "SELECT bbso_SONumber,finascop_stock_item_inventorydetails.stiid_id,br_Name,fpo_id,fpo_vendorName,fpo_poNumber,DATE_FORMAT(fpo_poDate,'%d-%m-%Y') AS fpo_poDate,"
                ...
                . "cpd_branch_id,br_Name,DATE_FORMAT(bbso_SODate,'%d-%m-%Y') AS bbso_SODate FROM retaline_B2B_SalesOrder "
                . "INNER JOIN finascop_stock_transfer_order ON fstr_id = bbso_id AND fsto_ordertype = 2 "
                . "INNER JOIN finascop_stock_transfer_order_details_barcodes ON finascop_stock_transfer_order_details_barcodes.fsto_id = finascop_stock_transfer_order.fsto_id "
                . "INNER JOIN finascop_stock_item_inventorydetails ON finascop_stock_item_inventorydetails.stiid_id = finascop_stock_transfer_order_details_barcodes.stiid_id "
                . "INNER JOIN finascop_purchase_order ON fpo_id = stiid_fpoid INNER JOIN finascop_branch ON finascop_branch.br_ID = cpd_branch_id {$search}{$where}{$searchitem} ORDER BY {$sort} {$dir} LIMIT $start,$limit ";
    ...
    public function handler($op) {
        ...
        switch ($op) {
            ...
            case 'B2BSalesDetailsReportsexportexcel':
                ...
                break;
            case 'getBranchName':
                $qry = $db->getMulipleData("SELECT br_ID,br_Name FROM finascop_branch WHERE br_status = 'Active' AND br_PyramidLevel = 4 ", true);
                if (!empty($qry)) {
