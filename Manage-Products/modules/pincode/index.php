...
//--
        ...
        // SECURITY: Raw filter — use buildSafeFilterQuery
        $allowedFields = ['pincode', 'area_name', 'city', 'state', 'pincode_status'];
        if (isset($_POST['filter'])) {
            $filter_qry .= buildSafeFilterQuery($_POST['filter'], $allowedFields, $db);
        }
        // LEGACY (remove after testing): if (isset($_POST['filter']) && $_POST['filter'] != '') {
            foreach ($_POST['filter'] as $key => $v) {
                $_allowedFilterFields = ['brName', 'dt_id', 'rownum', 'st_id'];
                ...
                switch ($v['data']['type']) {
                    case 'list':
                        if ($field == 'is_Active') {
                            $filterCon .= (($filterCon == "") ? " where " : " and ") . 'p.isActive' . " = '" . ($safeValue == 'Y' ? '1' : '0') . "'";
                        } else {
                            $filterCon .= (($filterCon == "") ? " where " : " and ") . $field . " like '" . $safeValue . "%'";
                        }
                        break;
                        
                    case 'string':
                        if ($field == 'brName') {
                    $brName = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(branch_id),0) FROM retaline_pincode WHERE branch_id IN (SELECT GROUP_CONCAT(br_ID) FROM finascop_branch WHERE br_Name LIKE '{$safeValue}%' )");
        ...
        $query = "select * from (select @cnt:=@cnt+1 as rownum,sel.* from
      	(select p.pincode, p.dst_id,s.st_name as st_id,b.br_Name as brName,IF(p.isActive=1,'Y','N')  AS is_Active,d.dst_Name as dt_id ,if(p.Has_Pickup=1,'Y','N') as pickup, if(p.Has_Delivery=1,'Y','N') as delivery from $prefix.retaline_pincode p inner join 
        $prefix.finascop_district d on p.dst_id = d.dst_Id inner join $prefix.finascop_state s on d.st_ID= s.st_ID inner join $prefix.finascop_branch b on p.branch_id= b.br_ID $filterCon order by $recSort $recSortDir)as sel )as sel2 
