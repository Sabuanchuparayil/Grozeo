...
//        }
        $db->query('begin');
        if ($adhocCount > 0) {
            $updatedDate = $db->getItemSafe("SELECT adhoc_updatedon FROM retaline_B2B_SalesRequest_adhoc WHERE adhoc_uniqueid = '?'", "i", [$_POST['b2bsr_UniqueID']]);
...
//for margin distributions
        ...
        $newupdatedDate = $db->getItemSafe("SELECT adhoc_updatedon FROM retaline_B2B_SalesRequest_adhoc WHERE adhoc_uniqueid = '?'", "i", [$_POST['b2bsr_UniqueID']]);
...
//        }
 ...
 FROM retaline_B2B_SalesRequest fp  {$filter_qry} where bbsr_id = '?'", "i", [$_POST['uniqueid']], true);
        } else {
 ...
 FROM retaline_B2B_SalesRequest_adhoc fp  {$filter_qry} where adhoc_uniqueid = '?'", "i", [$_POST['uniqueid']], true);
        }
...
//        }
        $db->query('begin');
        if ($adhocCount > 0) {
            $updatedDate = $db->getItemSafe("SELECT bbsr_SRUpdatedOn FROM retaline_B2B_SalesRequest WHERE bbsr_id = '?'", "i", [$_POST['b2bsr_UniqueID']]);
            if ($updatedDate == $_POST['sradhoc_updatedon']) {
...
//for margin distributions
        ...
        $newupdatedDate = $db->getItemSafe("SELECT bbsr_SRUpdatedOn FROM retaline_B2B_SalesRequest WHERE bbsr_id = '?'", "i", [$_POST['b2bsr_UniqueID']]);
        $status = $db->query('commit');
        if ($status == 1) {
            $msg = "'B2B Sales Request Item saved successfully.'";
            echo '{"success":true,"date":"' . $newupdatedDate . '","msg":' . $msg . '}';
