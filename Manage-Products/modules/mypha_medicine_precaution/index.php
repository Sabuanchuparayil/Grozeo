    ...
    case 'listadvicePrecaution':
        ...
        if (isset($_POST['filter'])) {
            ...
            foreach ($_POST['filter'] as $key => $val) {
                $_allowedFilterFields = ['advice_name', 'precaution_name'];
                if (!in_array($val['field'], $_allowedFilterFields)) continue;
                $safeValue = $db->sanitizeString($val['data']['value']);
               if ($val['field'] == 'advice_name') {
                    $advice_name = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(preadv_id),0) FROM mypha_precaution_advice WHERE advice_id IN (SELECT GROUP_CONCAT(advice_id) FROM mypha_safety_advice WHERE advice_name LIKE '{$safeValue}%')");
                    $search .= " AND preadv_id IN({$advice_name}) ";
                } else if ($val['field'] == 'precaution_name') {
                    $precaution_name = $db->getItemFromDB("SELECT COALESCE(GROUP_CONCAT(preadv_id),0) FROM mypha_precaution_advice WHERE precaution_id IN (SELECT GROUP_CONCAT(precaution_id) FROM mypha_safety_precaution WHERE precaution_name LIKE '{$safeValue}%')");
                    $search .= " AND preadv_id  IN({$precaution_name}) ";
                } else {
                    $search .= " and " . $val['field'] . " LIKE '%" . $safeValue . "%' ";
                }
            }
        }
