...
switch ($op) {
    ...
    case 'listStockPo':
        ...
        $search = " WHERE 1=1 AND fpo_centralStore = {$_SESSION['admin']->finascop_current_branch_id} ";
        $filter = $_POST['filter'];
    ...
    case 'addItemtoStock':
        ...
        if ($_REQUEST['poStockItemId'] == 'gift') {
            ...
            $packageDetails = $db->getFromDB("SELECT csb_package_type_name,csb_package_type_id,cs_nos,cs_package_type_name,cs_package_type_id,ds_nos,ds_package_type_name,cos_nos,cos_package_type_name,cos_package_type_id,cosb_package_type_id,dsb_package_type_id "
                    . "FROM finascop_stock_itemmaster WHERE stit_ID = {$_REQUEST['poStockItemId']}", true);
...
//new in finascop_stock_item_inventory    
        ...
        $query = "SELECT waqs_id,waqs_Name,waqs_Configuration FROM finascop_wallet_queue_settings WHERE waqs_id = 7";
        $wqSettings = $db->getFromDB($query, true);
        $transctionTemplate = json_decode($wqSettings['waqs_Configuration'], true);

        $hoBranchIDQuery = "SELECT br_ReferenceID FROM finascop_branch WHERE br_cpd = 0 and br_PyramidLevel = 1";
        $brRefID = $db->getItemFromDB($hoBranchIDQuery);

        //$amount = ($nofqty * $fsiiddata['stii_epraft']);
        $amount = ($leastSKUCount * $fsiidata['stii_leastSKUepr']);
        $transctionTemplate['cr']['Purchase']['amt'] = $amount;
        $transctionTemplate['cr']['Purchase']['br_ReferenceID'] = $brRefID;
        $transctionTemplate['dr']['CSStock']['amt'] = $amount;
        $br_ReferenceIDQuery = "SELECT br_ReferenceID FROM finascop_branch WHERE br_ID = " . intval($_SESSION['admin']->finascop_current_branch_id);
        $brRefID = $db->getItemFromDB($br_ReferenceIDQuery);
        $transctionTemplate['dr']['CSStock']['br_ReferenceID'] = $brRefID;

        $search = array("#ID#", "#NO#", "#AMT#");
        $replace = array($fpo_poNumber, $_REQUEST['fpo_id'], $amount);
        ...
        if (strcmp($transctionTemplate['comments'], '') != 0) {
            ...
            $fields = array(
                ...
                "waqu_comment" => $transctionTemplate['comments'], "waqu_SourceID" => intval($_REQUEST['fpo_id']),
