        ...
        switch ($ind) {
            case 1:
                ...
                switch ($_SESSION['admin']->finascop_typId) {
                    case 1:
                        $qry = "SELECT comp_id AS id, comp_shortname AS `name` from " . FINASCOP_DB . "finascop_company
                                WHERE comp_id IN (SELECT DISTINCT comp_Id from " . FINASCOP_DB . "`finascop_branch_company`) ";
                        break;
                    case 2:
                        $qry = "SELECT comp_id AS id, comp_shortname AS `name` from " . FINASCOP_DB . "finascop_company
                                WHERE comp_id IN ( SELECT comp_id  from " . FINASCOP_DB . "finascop_user_activecompanies WHERE UserId = {$_SESSION['admin']->Finascop_UserId}) ";
                        break;
                    case 3:
                        $qry = "SELECT comp_id AS id, comp_shortname AS `name` from " . FINASCOP_DB . "finascop_company
				...
				WHERE br_Id IN(SELECT br_Id from " . FINASCOP_DB . "finascop_user_activebranches WHERE UserId = {$_SESSION['admin']->Finascop_UserId})) ";
                    ...
                    case 4:
                        $qry = "SELECT comp_id AS id, comp_shortname AS `name` from " . FINASCOP_DB . "finascop_company
                                WHERE comp_id IN ( SELECT comp_id  from " . FINASCOP_DB . "finascop_branch_company 
				WHERE br_Id IN(SELECT br_Id from " . FINASCOP_DB . "finascop_user_auditingbranches  WHERE UserId = {$_SESSION['admin']->Finascop_UserId}))";
                        break;
                    default:
 ...
 */
                    ...
                    case 1:
                        $qry = "SELECT br_ID AS id, br_Name AS `name` from " . FINASCOP_DB . "finascop_branch
				WHERE br_ID IN (SELECT br_Id from " . FINASCOP_DB . "finascop_branch_company 
				WHERE comp_id = {$_POST['company']} AND br_Id IN(SELECT DISTINCT br_Id from " . FINASCOP_DB . "`finascop_branch_company`)) ";
                        break;
                    case 2:
                        $qry = "SELECT br_ID AS id, br_Name AS `name` from " . FINASCOP_DB . "finascop_branch
				WHERE br_ID IN (SELECT br_Id from " . FINASCOP_DB . "finascop_branch_company 
				WHERE comp_id = {$_POST['company']} ) ";
                        break;
                    case 3:
                        $qry = "SELECT br_ID AS id, br_Name AS `name` from " . FINASCOP_DB . "finascop_branch
				WHERE br_ID IN (SELECT br_Id from " . FINASCOP_DB . "finascop_branch_company 
				WHERE comp_id = {$_POST['company']} AND br_Id IN(SELECT br_Id from " . FINASCOP_DB . "finascop_user_activebranches WHERE UserId = {$_SESSION['admin']->Finascop_UserId})) ";
                        break;
                    case 4:
                        $qry = "SELECT br_ID AS id, br_Name AS `name` from " . FINASCOP_DB . "finascop_branch
				WHERE br_ID IN(SELECT br_Id from " . FINASCOP_DB . "finascop_branch_company 
				WHERE comp_id = {$_POST['company']} AND br_Id IN(SELECT br_Id from " . FINASCOP_DB . "finascop_user_auditingbranches  WHERE UserId = {$_SESSION['admin']->Finascop_UserId}))";
                        break;
                    default:
                        $qry = "SELECT br_ID AS id, br_Name AS `name` from " . FINASCOP_DB . "finascop_branch
				WHERE br_ID IN (SELECT br_Id from " . FINASCOP_DB . "finascop_branch_company 
    ...
    case 'listApproval':
        ...
        $finalisedcond = "";
        if ($_POST['approval_finalized'] == 'true') {
            $status_con = " (acet_Status = 3 OR  acet_Status = 4) ";
            $date_cond = " AND acet_Date BETWEEN '{$_POST['approval_record_from_date']}' AND  '{$_POST['approval_record_to_date']}' ";
            $finalisedcond = " OR (" . $status_con . $date_cond . ") ";
        }


        if ($_POST['approval_company'] > 0 || $_POST['approval_branch'] > 0 || $_POST['approval_type'] > 0) {

            if ($_POST['approval_company'] > 0) {
                $comp_con = " AND comp_id = {$_POST['approval_company']} ";
            }

            if ($_POST['approval_branch'] > 0) {
                $br_con = " AND branch_id = {$_POST['approval_branch']} ";
            }
            if ($_POST['approval_type'] > 0) {
                $type_con = " AND acet_TypeId = {$_POST['approval_type']} ";
            }
        }
        ...
        switch ($_SESSION['admin']->finascop_typId) {
            case 1:
                $branchqry = "(SELECT GROUP_CONCAT(DISTINCT br_Id) from " . FINASCOP_DB . "`finascop_branch_company`)";
                break;
            case 2:
                $branchqry = "(SELECT GROUP_CONCAT(DISTINCT br_Id) from " . FINASCOP_DB . "`finascop_branch_company` WHERE comp_id IN( SELECT comp_id  from " . FINASCOP_DB . "finascop_user_activecompanies WHERE UserId = {$_SESSION['admin']->Finascop_UserId})) ";
                break;
            case 3:
                $branchqry = "(SELECT GROUP_CONCAT(br_Id) from " . FINASCOP_DB . "finascop_user_activebranches WHERE UserId = {$_SESSION['admin']->Finascop_UserId}) ";
                break;
            case 4:
                $branchqry = "SELECT GROUP_CONCAT(br_Id) from " . FINASCOP_DB . "finascop_user_auditingbranches  WHERE UserId = {$_SESSION['admin']->Finascop_UserId}  ";
                break;
            default:
                $branchqry = "(SELECT GROUP_CONCAT(DISTINCT br_Id) from " . FINASCOP_DB . "`finascop_branch_company`)";
        }

        $branches = $db->getItemFromDB($branchqry);

        $countQuery = "Select 0";

        $listQuery = "SELECT acet_NO,acet_DocNO,DATE_FORMAT(acet_Date, '%d-%m-%y') AS acet_Date,IF(acet_Status = 3,1,0) AS Approved,IF(acet_Status = 3,1,IF(acet_Status = 4,1,0)) AS RollBack,
		...
		(SELECT GROUP_CONCAT(accled_LedgerName) from " . FINASCOP_DB . "finascop_accounts_ledger  inner join  " . FINASCOP_DB . "`finascop_accounts_transaction` fat on  accled_Ledger_Id = ledg_Id      WHERE fat.acet_NO = a.acet_NO 
		...
		IF(acet_Status = 1 or acet_Status = 2,if(acet_AssignedTo={$_SESSION['admin']->Finascop_UserId},0,1),-1) AS IsOtherAssignedUser,"
                . "IF(acet_Status = 1 or acet_Status = 2,if(acet_AssignedTo={$_SESSION['admin']->Finascop_UserId},'',"
                . "(SELECT CONCAT(FirstName,' ',LastName) FROM " . FINASCOP_DB . "`finascop_usr_profile` WHERE UserId = a.acet_AssignedTo)),'') AS AssignedToName,acet_EscalateComments as escalateComments,acet_RebutComments as rebutComments
		from " . FINASCOP_DB . "`finascop_accounts_entry` a 
		WHERE ((acet_Status = 0 and acet_QueueForRecon =0 ) {$finalisedcond} OR ((acet_Status = 1 or acet_Status = 2) and acet_QueueForRecon =0))		
		AND branch_id IN ({$branches})
                {$comp_con} {$br_con} {$type_con} {$filter_query}
