...
switch ($op) {
    ...
    case 'saveItemPODetails':
        ...
        $db->query('begin');
        if ($adhocCount > 0) {
//            $updatedDate = $db->getItemSafe("SELECT fcpo_updatedon FROM finascop_contractpo WHERE prereq_uniqueid = '?'", "i", [$_POST['fpopredet_uniqueid']]);
...
//echo 'fpot_leastSKUqty'.$fpot_leastSKUqty;
        ...
        $newupdatedDate = $db->getItemSafe("SELECT fcpo_updatedon FROM finascop_contractpo WHERE fcpo_uniqueid = '?'", "i", [$_POST['fpot_uniqueid']]);
 ...
 FROM finascop_contractpo fp  {$filter_qry} where fcpo_uniqueid = '?'", "i", [$_POST['uniqueid']], true);
        if (!empty($podata)) {
