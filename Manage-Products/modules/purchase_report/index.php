...
class PurchaseReportGeneration {
    ...
    public function _getPurchaseReturnReportGridData($postvar) {
        ...
        $listQuery = "SELECT fsb.stit_id as stit_id,br.br_Name as br_Name,fsi.stit_SKU as stit_SKU,fsbg_id,frbi_epr,frbi_leastSKUmrp,frbi_leastSKUepr,"
                ...
                . "INNER JOIN finascop_stock_itemmaster fsi ON fsb.stit_id=fsi.stit_ID "
                . "INNER JOIN finascop_branch br ON br.br_ID=fsb.branch_id {$search}{$where}{$searchitem} AND fsb.item_count > 0 ORDER BY {$sort} {$dir}  LIMIT $start,$limit";
        return array('countQuery' => $countQuery, 'listQuery' => $listQuery);
    }
    ...
    public function _getPurchaseReturnedReportGridData($postvar) {
        ...
        $countQuery = "SELECT COUNT(1) from " . FINASCOP_DB . " finascop_return_branch_inventory fsb "
                . "INNER JOIN finascop_stock_itemmaster fsi ON fsb.stit_id=fsi.stit_ID "
                . "INNER JOIN finascop_branch br ON br.br_ID=fsb.branch_id {$search}{$where}{$searchitem} AND item_returned > 0";
        ...
        $listQuery = "SELECT fsb.stit_id as stit_id,br.br_Name as br_Name,fsi.stit_SKU as stit_SKU,fsbg_id,frbi_epr,"
                ...
                . "FROM finascop_return_branch_inventory fsb "
                . "INNER JOIN finascop_stock_itemmaster fsi ON fsb.stit_id=fsi.stit_ID "
                . "INNER JOIN finascop_branch br ON br.br_ID=fsb.branch_id {$search}{$where}{$searchitem} AND item_returned > 0 ORDER BY fsb.stit_id asc LIMIT $start,$limit";
        return array('countQuery' => $countQuery, 'listQuery' => $listQuery);
    }
