...
switch ($op) {
    case 'listadvertisement':
       ...
       if (isset($_POST['filter'])) {
        ...
        $countQuery = "SELECT COUNT(*) FROM app_advertisements {$search} AND adv_status <> 2 ";
       $listQuery = "SELECT adv_id,adv_title,adv_status,adzone_name,aa.adzone_id AS ad_id FROM app_advertisements aa"
                . " INNER JOIN app_adzones az ON aa.adzone_id = az.adzone_id " 
                . " {$search} AND adv_status <> 2  ORDER BY {$sort} {$dir} limit $start,$limit"; 
    ...
    case 'advName':
        ...
        $listQuery = "SELECT adzone_id AS ad_id,adzone_name from app_adzones";
    ...
    case 'prtName':
        ...
        $listQuery = "SELECT stit_ID,stit_SKU from finascop_stock_itemmaster";
...
//            echo '{"totalCount":"0","data":[]}';
    ...
    case 'advertisementDetailsView':
        ...
        if ($adv_id || $ID) {
            ...
            $data = $db->getFromDB("SELECT CASE WHEN adv_offerType = 'Product' THEN (SELECT stit_SKU from finascop_stock_itemmaster WHERE stit_ID=adv_offerValueId) "
                    ...
                    . "ELSE (SELECT brand_name from mypha_productbrands WHERE brand_id=adv_offerValueId) END AS product1,"
                    . "adv_id,adv_title,adv_imageurl,aa.adzone_id AS ad_id,adzone_name,DATE_FORMAT(adv_startdate, '%d-%m-%Y') AS adv_startdate,"
                    . "DATE_FORMAT(adv_enddate, '%d-%m-%Y') AS adv_enddate,adv_offer,adv_offerpercent FROM app_advertisements aa "
    ...
    case 'advertisement_load':
        $adv_id = isset($_POST['adv_id']) ? intval($_POST['adv_id']) : 0;
        if ($adv_id) {
            $sql = "SELECT adv_id,adv_title,adv_status, adv_usageType,adv_imageurl,adzone_id AS ad_id,DATE_FORMAT(adv_startdate, '%Y-%m-%d') AS adv_startdate,DATE_FORMAT(adv_enddate, '%Y-%m-%d') AS adv_enddate,"
                    ...
                    . "CASE WHEN adv_offer = 'product' THEN (SELECT stit_SKU from finascop_stock_itemmaster WHERE stit_ID=adv_offerValueId) "
                    . "WHEN adv_offer = 'category' THEN (SELECT sub_category from mypha_productsubcategory WHERE sub_category_id=adv_offerValueId) "
                    . "ELSE (SELECT brand_name from mypha_productbrands WHERE brand_id=adv_offerValueId) END AS adv_offerValue_namess,"
